...
`zsx exec` reads a plan from stdin or `--file PLAN`. `-C ROOT` sets the primary project root for indexes, snaps, certificates, and unscoped searches. Relative filesystem paths stay rooted there. Explicit absolute `fs.read` and `fs.multiRead` paths may read exactly one canonical file outside the root per explicit read grant: the plan mints a bounded single-use grant with `zero.fs.read_grant({ path })` (or `zero.fs.compound("readGrant", { path })`), which canonicalizes the file, verifies it is a regular file outside the session root, and returns `{ grant_id, path, issued_at_unix_ms, expires_at_unix_ms }`. Every later external read re-verifies the canonical identity fresh; a symlink or path substitution fails closed, the grant is recorded on the read receipt (`read_grant` / `read_grants`), and mutation/search/list operations stay root-confined (the temporary re-root covers only the granted file's own parent for the duration of one call). One `fs.multiRead` call cannot mix relative and absolute paths, nor in-root and externally granted paths. Mutations remain primary-root jailed. `--timeout-ms N` overrides the 30000 ms default. Discover call shapes with `zero.help()` or `zero.help.search({query})`.
