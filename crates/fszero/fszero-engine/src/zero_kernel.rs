//! Typed FSZero implementation consumed directly by ZeroKernel.
//!
//! This module owns bytes and filesystem effects only. It exposes no command
    ...
    fn write_path(&self, relative: &Path) -> Result<PathBuf, EngineError> {
        ...
        if relative.is_absolute() {
            ...
            let canonical_parent = fs::canonicalize(parent).map_err(|error| {
                EngineError::new(
                    EngineErrorKind::Io,
                    format!("canonicalize write parent: {error}"),
                    false,
                )
            })?;
            return Ok(canonical_parent.join(target.file_name().ok_or_else(|| {
                EngineError::new(
                    EngineErrorKind::InvalidInput,
                    "write path has no name",
                    false,
                )
            })?));
        }
        ...
        Ok(canonical_parent.join(target.file_name().ok_or_else(|| {
            EngineError::new(
                EngineErrorKind::InvalidInput,
                "write path has no name",
                false,
...
fn apply_unique_patch(source: &[u8], patch: &str) -> Result<Vec<u8>, EngineError> {
    let value: serde_json::Value = serde_json::from_str(patch).map_err(|_| {
    ...
    if let Some(kind) = obj.get("kind").and_then(|v| v.as_str()) {
        ...
        if let Some(count) = obj
            .get("expectedCount")
            .or_else(|| obj.get("expected_count"))
        {
...
impl FileEngine for ZeroFileEngine {
    ...
    fn read(
        ...
        self.publish_snapshot(logical, bytes, metadata)
    }
    ...
    fn lookup(
        ...
        // Directories that must not be descended, to avoid burning the visited
        // budget inside VCS/store caches. Entries themselves are still counted
        // as one visited entry in the parent directory; we just skip recursion.
        ...
        // pc_3b55f2f33528: the old default of 20 silently truncated every
        // default lookup (a 33-file directory returned 20 entries with no
        // flag). FileEngine::lookup returns Vec<PathBuf> with no truncated
        // flag, so a prefix that looks complete is a lie. The unset default
        // still covers real directories; hitting the requested result cap or
        // LOOKUP_ENTRY_LIMIT with remaining work fails closed as Budget.
    ...
    fn apply(
        ...
        Self::cancelled(invocation)?;
        let target = self.write_path(&request.path)?;
        ...
        let before_metadata = before.as_ref().map(|snapshot| FileMetadata {
            mode: file_mode(&snapshot.2),
            modified_unix_ns: modified_ns(&snapshot.2),
        });
    ...
    fn restore(
        ...
        match receipt.before.as_ref() {
            ...
            None => {
                ...
                // from wedging the session forever (pc_1bc1981b8fee).
                let raw_target = if receipt.path.is_absolute() {
                    receipt.path.clone()
                } else {
                    self.root.join(&receipt.path)
                ...
                let target = match self.write_path_no_create(&receipt.path) {
                    Ok(p) => p,
                    Err(error) => {
                        if error.detail.contains("canonicalize write parent") {
                            return Ok(());
                        }
                        return Err(error);
                    }
...
mod line_range_tests {
    ...
    #[test]
    fn end_past_eof_clamps_to_last_line() {
        assert_eq!(
            line_range(b"alpha\nbeta\ngamma\n", "2:120").unwrap(),
            b"beta\ngamma\n",
        );
...
#[cfg(all(test, unix))]
mod tests {
    use super::same_file_version;
    use crate::ZeroFileEngine;
    use std::path::{Path, PathBuf};
    use std::time::{Duration, UNIX_EPOCH};
    use zero_abi::{
        CancellationProbe, EngineCallContext, EngineInvocation, FileEffectKind, FileEffectRequest,
        FileEngine, FileReadRequest, KernelBudget, LookupOptions, ReadOptions,
    };
    ...
    fn test_invocation(root: &Path) -> EngineInvocation {
        EngineInvocation {
    ...
    #[test]
    fn lookup_returns_every_entry_beyond_the_old_default_cap() {
        let root = tempfile::tempdir().unwrap();
        let engine =
            ZeroFileEngine::open(root.path(), &root.path().join(".zerostack"), "contract").unwrap();
        let dir = root.path().join("many");
        std::fs::create_dir_all(&dir).unwrap();
        for i in 0..40 {
            std::fs::write(dir.join(format!("f{i:02}.txt")), "x").unwrap();
        }
        // Tool-internal store directory: must not appear in discovery output.
        std::fs::create_dir_all(dir.join(".asgrep")).unwrap();
        std::fs::write(dir.join(".asgrep").join("index.db"), "x").unwrap();

        let paths = engine
            .lookup(
                &test_invocation(root.path()),
                PathBuf::from("many"),
                LookupOptions::default(),
            )
            .unwrap();
        assert_eq!(paths.len(), 40, "all 40 files must be returned");
        assert!(
            paths
                .iter()
                .all(|path| !path.to_string_lossy().contains(".asgrep")),
            "tool-internal dirs must be hidden: {paths:?}"
        );
    }
    ...
    fn lookup_recurses_only_when_requested() {
        ...
        std::fs::write(root.path().join("tree/top.txt"), "top").unwrap();
        std::fs::write(root.path().join("tree/nested/deep.txt"), "deep").unwrap();
    ...
    #[test]
    fn same_length_replacement_has_distinct_file_identity() {
        ...
        assert!(!same_file_version(&first, &second));
    }

    #[test]
    fn restore_recovers_bytes_mode_and_modified_time() {
        use std::os::unix::fs::PermissionsExt;

        let root = tempfile::tempdir().unwrap();
        let engine =
            ZeroFileEngine::open(root.path(), root.path().join(".zerostack"), "contract").unwrap();
        let relative = PathBuf::from("rollback.txt");
        ...
        std::fs::OpenOptions::new()
            ...
            .unwrap();
        let original_metadata = std::fs::metadata(&target).unwrap();
        let invocation = test_invocation(root.path());
        let snapshot = engine
            .read(
                &invocation,
                FileReadRequest {
    ...
    #[test]
    fn restore_fails_when_preexisting_parent_disappears() {
        let root = tempfile::tempdir().unwrap();
        let engine =
            ZeroFileEngine::open(root.path(), root.path().join(".zerostack"), "contract").unwrap();
        let relative = PathBuf::from("nested/rollback.txt");
        let target = root.path().join(&relative);
        ...
        let error = engine.restore(&invocation, &receipt).unwrap_err();
        assert_eq!(error.kind, zero_abi::EngineErrorKind::Io);
        assert!(error.detail.contains("rollback parent"));
    }
}
