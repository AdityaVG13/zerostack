...
pub fn validate_plan(plan: &str, max_bytes: usize) -> Result<(), PlanError> {
    if plan.trim().is_empty() { return Err(PlanError::Empty); }
    if plan.len() > max_bytes { return Err(PlanError::TooLarge { actual: plan.len(), maximum: max_bytes }); }
    if plan.contains('\0') { return Err(PlanError::Nul); }
    Ok(())
}
...
pub fn wrap_plan(plan: &str, max_bytes: usize) -> Result<String, PlanError> {
    ...
    let encoded = serde_json::to_string(plan).map_err(|error| PlanError::Encoding(error.to_string()))?;
    Ok(format!(r#"(async () => {{
"use strict";
...
impl fmt::Display for PlanError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooLarge { actual, maximum } => write!(f, "plan is {actual} bytes; maximum is {maximum}"),
