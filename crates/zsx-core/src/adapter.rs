...
impl AdapterError {
    ...
    pub fn new(
        ...
        let error = match WorkerError::new(kind, message.clone(), retryable) {
            Ok(error) => error,
            Err(rejected) => WorkerError::new("internal", format!("{rejected}: {message}"), false)
                .expect("internal is a closed RW5 kind"),
        };
        Self {
            error: Box::new(error),
