...
impl DomainAdapter for TokenZeroAdapter {
    ...
    fn call(&self, call: AdapterCall<'_>) -> Result<AdapterResponse, AdapterError> {
        ...
        let raw_outcome = with_host_wall_deadline_and_cancel(
            WallDeadline::from_elapsed_ms(0, remaining_ms),
            call.cancellation.as_atomic(),
            || self.dispatch(request),
        );
        let outcome = match raw_outcome {
            Err(error) if call.cancellation.is_cancelled() => {
                let msg = error.message.to_lowercase();
                let is_cancelled = error.kind == DomainErrorKind::Cancelled
                    || msg.contains("cancelled")
                    || msg.contains("cancellation")
