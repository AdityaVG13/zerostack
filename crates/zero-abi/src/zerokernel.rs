...
mod tests {
    use super::*;
    use crate::decision::{DecisionRequired, ObservationClass};
    use serde_json::json;

    fn sample_budget() -> FiniteBudget { FiniteBudget::new(5000, 5000, 64 * 1024 * 1024, 32).unwrap() }
    fn sample_policy() -> ReturnPolicy { ReturnPolicy::new(ReturnKind::Inline, 512).unwrap() }
    fn sample_roots() -> RootBindings {
        RootBindings::new(None, "proj_abc123".into(), None, Some("cap_root_123".into()), None).unwrap()
    }
    fn sample_snapshot() -> RootSnapshot {
        RootSnapshot { workspace_root: None, project_root: "proj_abc123".into(), session_root: Some("sess_1".into()) }
    }
    fn sample_handles() -> ExactHandles { ExactHandles { session_handle: Some("sess_1".into()), continuation_handle: None } }
    fn sample_preflight() -> PreflightReport { PreflightReport { ok: true, checked_roots: vec!["proj_abc123".into()], warnings: vec![], errors: vec![] } }
    fn sample_ledger() -> KernelResourceLedger { KernelResourceLedger { wall_ms_used: 10, cpu_ms_used: 5, calls_made: 1, bytes_out: 100 } }
    fn sample_evidence_unchanged() -> RootEvidence {
        let snap = sample_snapshot();
        RootEvidence { before: snap.clone(), after: snap, unchanged: true, successor_root: None }
    }
    fn sample_evidence_changed() -> RootEvidence {
        RootEvidence {
            before: RootSnapshot { workspace_root: None, project_root: "proj_abc123".into(), session_root: Some("sess_1".into()) },
            after: RootSnapshot { workspace_root: None, project_root: "proj_abc123".into(), session_root: Some("sess_2".into()) },
            unchanged: false,
    ...
    fn round_trip_request_canonical() {
        let req = ZerokernelExecuteRequest::new("return 42;".into(), Some("sess_1".into()), sample_budget(), sample_policy(), sample_roots()).unwrap();
        ...
        let embedded = ZerokernelExecuteRequest::new("return 42;".into(), Some("sess_1".into()), sample_budget(), sample_policy(), sample_roots()).unwrap();
        let oneshot = ZerokernelExecuteRequest::new("return 42;".into(), Some("sess_1".into()), sample_budget(), sample_policy(), sample_roots()).unwrap();
        assert_eq!(embedded.canonical_json(), oneshot.canonical_json());
    }

    #[test]
    fn round_trip_response_completed_and_decision() {
        let completed = ZerokernelExecuteResponse::completed(sample_handles(), sample_preflight(), sample_ledger(), sample_evidence_changed(), json!({"ok":true,"value":42})).unwrap();
        let bytes = completed.canonical_bytes();
        assert_eq!(completed, ZerokernelExecuteResponse::from_canonical_bytes(&bytes).unwrap());

        let obs = ObservationClass::new("test.class").unwrap();
        let decision = DecisionRequired { decision_id: "d1".into(), observation_class: obs, question: "choose?".into(), choices: vec!["a".into(), "b".into()], observed_value: "c".into() };
        let dr = ZerokernelExecuteResponse::decision_required(sample_handles(), sample_preflight(), sample_ledger(), sample_evidence_unchanged(), decision).unwrap();
        let bytes2 = dr.canonical_bytes();
        assert_eq!(dr, ZerokernelExecuteResponse::from_canonical_bytes(&bytes2).unwrap());
    }

    #[test]
    fn round_trip_failed_proves_unchanged() {
        let failed = ZerokernelExecuteResponse::failed(sample_handles(), sample_preflight(), sample_ledger(), sample_evidence_unchanged()).unwrap();
    ...
    fn unknown_fields_fail_closed() {
        ...
        let req = ZerokernelExecuteRequest::new("p".into(), None, budget, policy, roots).unwrap();
    ...
    fn mutation_effect_daemon_fields_rejected() {
        for field in ["mutation","effect","daemon","pool","background_pool","write_authority"] {
            ...
            assert!(serde_json::from_value::<ZerokernelExecuteRequest>(v).is_err(), "field {field} should be rejected");
        }
    }
    ...
    fn invalid_root_combinations_fail() {
        ...
        let req = ZerokernelExecuteRequest::new("prog".into(), None, sample_budget(), sample_policy(), roots);
        assert!(req.is_err());
    }
    ...
    fn failure_with_successor_root_rejected() {
        ...
        let res = ZerokernelExecuteResponse::failed(sample_handles(), sample_preflight(), sample_ledger(), evidence);
        assert!(res.is_err());
        // also changed evidence with Failed should fail
        let mut evidence2 = sample_evidence_changed();
        evidence2.unchanged = false;
        let res2 = ZerokernelExecuteResponse::failed(sample_handles(), sample_preflight(), sample_ledger(), evidence2);
        assert!(res2.is_err());
    }

    #[test]
    fn wrong_abi_version_rejected() {
        let mut req = ZerokernelExecuteRequest::new("p".into(), None, sample_budget(), sample_policy(), sample_roots()).unwrap();
