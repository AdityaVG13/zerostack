...
impl Default for ZeroTelemetryV1 {
    fn default() -> Self {
        Self { schema: TelemetrySchema::V1, prevented_files: 0, prevented_bytes: 0 }
    }
}
...
impl ZeroTelemetryV1 {
    ...
    pub fn checked_accumulate(
        ...
        *counter = counter.checked_add(amount).ok_or(TelemetryOverflow { field })?;
        Ok(())
    }
    ...
    pub fn checked_merge(&mut self, other: Self) -> Result<(), TelemetryOverflow> {
        let prevented_files = self
            ...
            .ok_or(TelemetryOverflow { field: TelemetryCounter::PreventedFiles })?;
        let prevented_bytes = self
            .prevented_bytes
            .checked_add(other.prevented_bytes)
            .ok_or(TelemetryOverflow { field: TelemetryCounter::PreventedBytes })?;
...
mod tests {
    ...
    fn shared_telemetry_serialization_is_canonical_and_deterministic() {
        let telemetry = ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 2, prevented_bytes: 9 };
        assert_eq!(serde_json::to_string(&telemetry).unwrap(),
            r#"{"schema":"zero-telemetry/v1","prevented_files":2,"prevented_bytes":9}"#);
        assert_eq!(serde_json::to_value(TelemetrySchema::V1).unwrap(), json!("zero-telemetry/v1"));
    }
    ...
    fn shared_telemetry_checked_accumulation_reports_typed_overflow() {
        let mut telemetry = ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: u64::MAX, prevented_bytes: 0 };
        assert_eq!(telemetry.checked_accumulate(TelemetryCounter::PreventedFiles, 1),
            Err(TelemetryOverflow { field: TelemetryCounter::PreventedFiles }));
        assert_eq!(telemetry.prevented_files, u64::MAX);
    }

    #[test]
    fn shared_telemetry_merge_is_transactional_on_overflow() {
        let mut telemetry = ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 4, prevented_bytes: u64::MAX };
        let original = telemetry;
        assert_eq!(telemetry.checked_merge(ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 1, prevented_bytes: 1 }),
            Err(TelemetryOverflow { field: TelemetryCounter::PreventedBytes }));
    ...
    fn shared_telemetry_merge_is_deterministic() {
        let rows = [
            ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 1, prevented_bytes: 5 },
            ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 2, prevented_bytes: 7 },
            ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 3, prevented_bytes: 11 },
        ];
        let mut forward = ZeroTelemetryV1::default();
        for row in rows { forward.checked_merge(row).unwrap(); }
        let mut reverse = ZeroTelemetryV1::default();
        for row in rows.into_iter().rev() { reverse.checked_merge(row).unwrap(); }
        assert_eq!(forward, reverse);
        assert_eq!(forward, ZeroTelemetryV1 { schema: TelemetrySchema::V1, prevented_files: 6, prevented_bytes: 23 });
    }
}
