...
//! Typed GraphZero implementation consumed directly by ZeroKernel. `z.find` is the typed structural
//! entry point. Natural/pattern/semantic modes execute ast-sgrep in-process; graph relationship
//! modes execute GraphZero's typed query router.
...
impl ZeroStructuralEngine {
    /// Authoritative ZeroKernel index worker limit. AST and graph indexing
    /// through this adapter use one worker to avoid CPU amplification during
    /// kernel activation and the first structural query.
    pub const INDEX_WORKERS: usize = 1;
    /// Namespace for fallback indexes when the legacy database was written by a newer,
    /// incompatible ast-sgrep schema. Bump with the pinned ast-sgrep on-disk schema.
    const AST_SGREP_SCHEMA_NAMESPACE: &str = "schema-12";
    ...
    fn deadline(invocation: &EngineInvocation) -> Option<Instant> {
        if invocation.context.deadline_unix_ms == u64::MAX {
            return None;
        }
    ...
    fn ast_schema_fallback_path(index_path: &Path) -> PathBuf {
        let stem = index_path
            .file_stem()
            .and_then(|stem| stem.to_str())
            .unwrap_or("index");
        index_path.with_file_name(format!("{stem}.{}.db", Self::AST_SGREP_SCHEMA_NAMESPACE))
    }
    ...
    fn ast_file_filter(&self, query: &StructuralQuery) -> Result<Option<String>, EngineError> {
        ...
        if path.is_absolute() {
            return Err(EngineError::new(
                ...
                "z.find is confined to the kernel root and requires a workspace-relative path",
                false,
            ));
        }
        ...
        let filter = path.to_string_lossy().into_owned();
        if !Self::is_glob_filter(&filter)
            && let Ok(canonical) = std::fs::canonicalize(self.repo_root.join(path))
            && !canonical.starts_with(&self.repo_root)
        {
            return Err(EngineError::new(
                EngineErrorKind::OutsideWorkspace,
                format!("z.find path escapes the kernel root: {}", path.display()),
                false,
            ));
        }
        Ok(Some(filter))
    ...
    fn ast_scope(&self, filter: Option<&str>) -> Option<(PathBuf, String, Option<String>)> {
        ...
        let target = std::fs::canonicalize(self.repo_root.join(relative)).ok()?;
        ...
        let parent_prefix = relative.parent().unwrap_or_else(|| Path::new(""));
        let filename = target.file_name()?.to_string_lossy().into_owned();
        Some((
            target.parent()?.to_path_buf(),
            parent_prefix.to_string_lossy().into_owned(),
            Some(filename),
        ))
    }
    ...
    fn ensure_ast_index(
        ...
        index_root: &Path,
        index_path: &Path,
        scoped_file: Option<&str>,
    ) -> Result<(IndexStats, PathBuf), EngineError> {
        ...
        let index_options = |path: &Path| IndexOptions {
        ...
        let (mut indexer, active_index_path) = match Indexer::new(index_options(index_path)) {
            Ok(indexer) => (indexer, index_path.to_path_buf()),
            Err(error) => {
                let detail = error.to_string();
                if !detail.contains("index schema version")
                    || !detail.contains("newer than supported")
                {
                    return Err(EngineError::new(
                        EngineErrorKind::Internal,
                        format!(
                            "ast-sgrep open scoped index root={} index={}: {detail}",
                            index_root.display(),
                            index_path.display()
                        ),
                        true,
                ...
                let fallback = Self::ast_schema_fallback_path(index_path);
                let indexer = Indexer::new(index_options(&fallback)).map_err(|fallback_error| {
                    EngineError::new(
                        EngineErrorKind::Internal,
                        format!(
                            "ast-sgrep open schema fallback root={} index={} after incompatible {}: {fallback_error}",
                            index_root.display(),
                            fallback.display(),
                            index_path.display()
                        ),
        ...
        let indexed = if let Some(file) = scoped_file {
            let updated = indexer
                .update_paths(&[PathBuf::from(file)])
                .map_err(|error| ast_stage_error("refresh exact-file index", error));
            updated.and_then(|updated| {
                indexer
                    .flush_deferred_rebuilds()
                    .map_err(|error| ast_stage_error("rebuild exact-file sidecars", error))?;
                Ok(IndexStats {
                    files_indexed: updated.files_indexed,
                    files_skipped: updated.files_skipped,
                    files_removed: updated.files_removed,
                    files_failed: updated.files_failed,
                    ..IndexStats::default()
                })
            })
        } else {
    ...
    fn ast_query(
        ...
        let (index_stats, index_path) = self.ensure_ast_index(
            invocation,
            query,
            &index_root,
            &index_path,
            scoped_file.as_deref(),
        )?;
    ...
    fn graph_query(
        ...
        let mut snapshot =
            graphzero_store::Snapshot::open_cached(&self.graph_store_root, Some(&self.repo_root))
                .map_err(graph_error)?;
        // Each surface has one canonical request field. Supplying both can fail
        // validation and hide otherwise valid rows during conversion.
    ...
    fn convert_graph_response(
        ...
        // Symbol and context surfaces use decl_ref or capsule instead of row lists.
        // Synthesize definition and usage hits before applying the public limit.
        if hits.is_empty() {
            ...
            if let Some(capsule) = response.capsule.as_ref() {
                ...
                if hits.len() <= 1 {
                    // Use refs_footer evidence when destinations add no hit beyond decl_ref.
...
impl ZeroStructuralEngine {
    /// lens verdict over one rooted definition/symbol query.
    fn lens_symbol(
        &self,
        invocation: &EngineInvocation,
    ...
    /// Bounded reverse (caller) closure of one locus symbol over the live graph, walking the
    /// same reverse-index evidence the callers surface uses. Returns the closure edges and
    /// whether the walk stayed within the bound; `None` when the symbol is absent from the symbol table.
...
/// Explicit semantic decision-gap evidence carried by the graph response. Only marked gaps
/// count: `decision_gaps` entries in the capsule/absence ledger, or an error prefixed
/// `SEMANTIC_DECISION_GAP:`. Absence certificates and ordinary index errors are not decision gaps.
...
/// The blob hash from a `z://blob/<hex>#B<start>-<end>` span ref.
fn blob_hash_from_evidence_ref(evidence_ref: &str) -> Option<String> {
    let hex = evidence_ref.strip_prefix("z://blob/")?.split('#').next()?;
