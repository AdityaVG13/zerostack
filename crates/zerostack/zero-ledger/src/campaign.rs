//! Allocates cold and warm costs across reuse campaigns and break-even horizons.
//! Every per-use figure includes cold indexing and object-formation costs.
...
impl ReuseCampaign {
    ...
    pub fn total_campaign_cost(&self) -> u128 {
        u128::from(self.cold_build_cost)
            + u128::from(self.per_use_cost) * u128::from(self.reuse_count)
    }
    ...
    pub fn campaign_surplus(&self) -> i128 {
        i128::try_from(self.alternative_total_cost()).expect("fits i128")
            - i128::try_from(self.total_campaign_cost()).expect("fits i128")
    }

    /// Amortized cold-build share per use as the reduced fraction `C / n`. This is the
    /// allocation of the indexing/object-formation cost over the actual reuse campaign.
    ...
    pub fn allocated_per_use(&self) -> (u128, u128) {
        let numerator = u128::from(self.cold_build_cost)
            + u128::from(self.per_use_cost) * u128::from(self.reuse_count);
    ...
    /// Strict break-even horizon: the smallest number of uses with `C + n * W <= n * A`,
    /// i.e. `ceil(C / s)`, exact. Zero uses when the cold build is zero (break-even at once).
    ...
    /// Q99 break-even horizon over the *actual* campaign's per-use savings sample. The sample's 99th
    /// percentile is the exact rank `ceil(0.99 * len)` in ascending order (1-based); the horizon is
    /// `ceil(C / q99)`.
    pub fn q99_break_even_uses(
        &self,
        observed_per_use_savings: &[u64],
    ...
    /// Verifies that a claimed campaign total includes the cold build. Any claim below the cold-build
    /// cost omits it and is refused: a warm-run figure can never be presented as the campaign cost.
    pub fn check_claim_includes_cold_build(
        &self,
        claimed_total: u128,
...
impl fmt::Display for CampaignError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::WarmRunClaimOmitsColdBuild {
                claimed,
                cold_build,
            } => write!(
                f,
