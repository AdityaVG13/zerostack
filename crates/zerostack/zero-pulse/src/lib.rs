...
pub const PULSE_SYNC_SCHEMA_VERSION: &str = "zerostack.pulse-sync";
...
/// Pulse count class for a grammar-valid tokenizer id. `estimator:` is
/// approximate. `tiktoken:` is bundled BPE (kernel-certified for that
/// vocab, never Pulse ExactTokenizerIdentity). `exact` is only `provider/model@hex`.
...
pulse_structs! {
    PulseEvent {
        ...
        latency_ms u128;
        /// `tool_call` stores the first 64 bits (16 hex characters) of its source
        /// hint's SHA-256 here. Direct construction/deserialization is not
        /// validated, so callers must never place raw payloads in this correlatable field.
        ...
        /// Exact recovery handles used as RACC join keys. Stored verbatim in the
        /// local ledger when supplied and potentially correlatable with local payloads.
...
pub fn default_ledger_path(root: &Path) -> PathBuf {
    ResolvedStore::resolve_from_process(root, Engine::TokenZero, &["ZEROSTACK_SHARED_STORE"])
        .engine_dir()
        .join("pulse/events.jsonl")
}

/// Pulse ledger path under an already resolved unified state root.
pub fn ledger_path_for_store_root(store_root: &Path) -> PathBuf {
    store_root
        .join(Engine::TokenZero.dir_name())
        .join("pulse/events.jsonl")
}

/// Record kernel TokenAccounting under its exact state root and call identity.
pub fn record_kernel_accounting(
    store_root: &Path,
    session_id: &str,
    call_id: &str,
    tool: &str,
    accounting: &zero_abi::TokenAccounting,
) -> IoResult<()> {
    let checked_tokens = |field: &str, value: u64| {
        usize::try_from(value).map_err(|_| {
            IoError::new(
                ErrorKind::InvalidInput,
                format!("{field} token count exceeds this platform's usize"),
            )
        })
    };
    let event = PulseEvent::tool_call(
        tool,
        "kernel",
        checked_tokens("raw", accounting.billed)?,
        checked_tokens("visible", accounting.visible)?,
        checked_tokens("recovery", accounting.cached)?,
        0,
        0,
        None,
    )
    ...
    .with_tokenizer_id(&accounting.tokenizer)
    .map_err(|message| IoError::new(ErrorKind::InvalidInput, message))?;
    record_event(&ledger_path_for_store_root(store_root), &event)
}
...
// WouldBlock only for bounded wait paths; direct lock acquisition still returns the platform error.
...
// Session-ledger pricing uses resident token mass times remaining turns.
