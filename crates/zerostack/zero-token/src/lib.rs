...
//! TokenZero compression and accounting behind ZeroKernel's typed [`zero_abi::TokenEngine`].
...
use zero_abi::{
    ...
    ExpandOptions, HANDLE_DIGEST_BYTES, ProjectionRequest, ProjectionResult, TokenAccounting,
    TokenEngine, ZERO_HANDLE_PREFIX, ZeroHandle,
};
use zero_store::{SelectionIndex, ZeroCas, ZeroObjectMetadata};

const EXACT_MARKER_BYTE_LEN: usize =
    "\nexact: ".len() + ZERO_HANDLE_PREFIX.len() + HANDLE_DIGEST_BYTES;

/// Output-contract 4-tuple for [`TokenAccounting`]. `raw` is billed source mass,
/// `visible`/`spent` are presented mass, `recovered` is cache/recovery mass (`cached`,
/// 0 until a cache hit is charged). This mapping is the account golden, not a second counter.
...
/// `spent` is visible plus recovered/cached mass; using visible alone hides expand charges.
...
impl ZeroTokenEngine {
    ...
    /// Open without reading `ZEROSTACK_MODEL`.
    ...
    fn clamp_visible_to_budget<'a>(
            ...
            && let Ok(bpe) = tiktoken_rs::bpe_for_model(model)
        {
            ...
            let mut end = usize::try_from(max_tokens)
                .unwrap_or(usize::MAX)
                .min(tokens.len());
            while end > 0 {
                let Ok(decoded) = bpe.decode(&tokens[..end]) else {
                    end -= 1;
                    continue;
                };
                ...
                let prefix = &text[..decoded.len()];
                let prefix_accounting = self.count(prefix.as_bytes());
                if prefix_accounting.billed <= max_tokens {
                    return (prefix, prefix_accounting, true);
                }
                end /= 2;
            }
        } else {
            let prefix = tokenzero_core::pack_to_token_boundary_for_model(
                text,
                usize::try_from(max_tokens).unwrap_or(usize::MAX),
                self.model_id.as_deref(),
            );
            return (prefix, self.count(prefix.as_bytes()), true);
        }
...
impl TokenEngine for ZeroTokenEngine {
    ...
    fn project(
        ...
        if limit < EXACT_MARKER_BYTE_LEN {
            return Err(EngineError::new(
        ...
        let marker = format!("\nexact: {handle}");
        let (visible, visible_source_bytes) = match std::str::from_utf8(&request.bytes) {
            Ok(source) => {
                let visible = bounded_utf8(source, &marker, limit);
                let source_bytes = visible.strip_suffix(&marker).map_or(0, str::len) as u64;
                (visible, source_bytes)
            }
            Err(_) => (bounded_utf8("", &marker, limit), 0),
        };
        let visible_count = self.count(visible.as_bytes());
        // Never-worse vs the raw payload: a handle capsule that costs more tokens
        // than sending the source is not a projection. Pass through even when the
        // byte budget is tight — TokenZero's authority is tokens, not framing bytes.
    ...
    fn compress(
        ...
        // Auto+oversized must pick the same honest digest family at every budget, then clamp.
        ...
        // Reject wrappers billed above raw before applying the visible limit.
        // Truncation must not turn wrapper overhead into reported savings.
