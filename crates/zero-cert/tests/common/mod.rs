...
impl Resolver for Residents<'_> {
    fn resolve<'a>(&'a self, object_id: &ObjectId) -> Option<&'a [u8]> {
        self.objects.iter().copied().find(|bytes| zero_abi::sha256(bytes) == object_id.0)
    }
    fn trusted_operator_version<'a>(&'a self, id: &str) -> Option<&'a str> { (id == "read-span").then_some("1") }
    fn trusted_parser_version<'a>(&'a self, id: &str) -> Option<&'a str> { (id == "tree-sitter").then_some("1") }
    fn trusted_index_version<'a>(&'a self, id: &str) -> Option<&'a str> { (id == "zero-index").then_some("2") }
    fn resolve_mutation_receipt<'a>(&'a self, journal_id: &Digest, sequence: u64) -> Option<&'a [u8]> {
        self.mutation_receipts.iter().find_map(|(id, seq, bytes)| (id == journal_id && *seq == sequence).then_some(*bytes))
    }
    fn resolve_aggregate_receipt<'a>(&'a self, snapshot_id: &Digest) -> Option<&'a [u8]> {
        self.aggregate_receipts.iter().find_map(|(id, bytes)| (id == snapshot_id).then_some(*bytes))
    }
}

pub fn object_id(bytes: &[u8]) -> ObjectId { ObjectId(zero_abi::sha256(bytes)) }
pub fn provenance() -> Provenance {
    Provenance { parser_id: "tree-sitter".into(), parser_version: "1".into(), index_id: "zero-index".into(), index_version: "2".into(), operator_id: "read-span".into(), operator_version: "1".into() }
}
pub fn span(bytes: &[u8], start: usize, len: usize) -> SpanRef {
    SpanRef { object_id: object_id(bytes), object_digest: zero_abi::sha256(bytes), byte_start: start as u64, byte_len: len as u64, span_digest: zero_abi::sha256(&bytes[start..start + len]) }
}
