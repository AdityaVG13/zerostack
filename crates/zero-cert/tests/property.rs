...
fn ordered_domain_digest_and_overlapping_matches_are_deterministic() {
    ...
    let first = b"aaa".as_slice(); let second = b"ba".as_slice();
    let objects = vec![common::object_id(first), common::object_id(second)];
    let snapshot = domain_snapshot_digest(&objects, "zero-index", "2");
    let mut reversed = objects.clone(); reversed.reverse();
    assert_ne!(snapshot, domain_snapshot_digest(&reversed, "zero-index", "2"));
    assert_ne!(snapshot, domain_snapshot_digest(&objects, "zero-index", "3"));
    let certificate = EvidenceCertificate {
        query: Query::ExactSearchDomain { pattern: Cow::Borrowed(b"aa"), objects: objects.clone(), snapshot_id: snapshot, index_id: "zero-index".into(), index_version: "2".into() },
        spans: vec![common::span(first, 0, 2), common::span(first, 1, 2)],
        payload: Cow::Borrowed(b"aaaa"), provenance: common::provenance(),
        completeness: CompletenessWitness::ExactSearchDomain { operator: OperatorLock { operator_id: "read-span".into(), operator_version: "1".into() }, pattern: Cow::Borrowed(b"aa"), objects, snapshot_id: snapshot, index_id: "zero-index".into(), index_version: "2".into(), match_count: 2 },
        input_token_cost: 0, backend_work_units: 0,
    };
    ...
    let mut omitted = certificate.clone(); omitted.spans.pop(); omitted.payload.to_mut().truncate(2);
    assert!(matches!(verify(&omitted, &residents), Err(VerificationError::InvalidCompleteness)));
}
