...
//! `LayerValidityLedgerV1` (mark L3 loss / complete refetch); engines do not
...
//! [`run_scrub_v1`] with an idle-age filter for background passes.
...
pub const SCRUB_SCHEMA_VERSION_V1: u16 = 1;
/// Maximum objects examined in one pass, so a background pass is bounded.
pub const SCRUB_MAX_OBJECTS_PER_PASS_V1: usize = 262_144;
/// Objects larger than this are refused by the pass (aligned with the CAS
/// size policy).
pub const SCRUB_MAX_OBJECT_BYTES_V1: u64 = CAS_MAX_OBJECT_BYTES;

const SCRUB_RECEIPT_DOMAIN_V1: &[u8] = b"zerostack.scrub.receipt.v1\0";

/// Configuration of one scrub pass.
#[derive(Clone, Copy, Debug)]
pub struct ScrubConfigV1 {
    /// Upper bound on objects examined (defaults to
    /// [`SCRUB_MAX_OBJECTS_PER_PASS_V1`]).
    pub max_objects: Option<usize>,
    /// Per-object size ceiling (defaults to
    /// [`SCRUB_MAX_OBJECT_BYTES_V1`]).
...
impl Default for ScrubConfigV1 {
    fn default() -> Self {
        Self {
            max_objects: None,
            max_object_bytes: SCRUB_MAX_OBJECT_BYTES_V1,
...
pub struct ScrubFindingV1 {
    pub identity: String,
    pub kind: ScrubFindingKindV1,
    pub detail: String,
}
...
pub struct ScrubReceiptV1 {
    ...
    pub findings: Vec<ScrubFindingV1>,
}
impl ScrubReceiptV1 {
    pub fn validate(&self) -> Result<(), ScrubErrorV1> {
        if self.schema_version != SCRUB_SCHEMA_VERSION_V1 {
            return Err(ScrubErrorV1::SchemaViolation(
    ...
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, ScrubErrorV1> {
        self.validate()?;
        let value = serde_json::to_value(self)
            .map_err(|error| ScrubErrorV1::SchemaViolation(error.to_string()))?;
        Ok(canonical_json(&value).into_bytes())
    }
    pub fn digest(&self) -> Result<DigestV1, ScrubErrorV1> {
        let bytes = self.canonical_bytes()?;
        let mut bound = Vec::with_capacity(SCRUB_RECEIPT_DOMAIN_V1.len() + 8 + bytes.len());
        bound.extend_from_slice(SCRUB_RECEIPT_DOMAIN_V1);
...
impl std::fmt::Display for ScrubErrorV1 {
...
impl std::error::Error for ScrubErrorV1 {}
...
pub fn run_scrub_v1(
    store_root: &Path,
    config: &ScrubConfigV1,
    producer_id: &str,
    operation_id: &str,
) -> Result<ScrubReceiptV1, ScrubErrorV1> {
    ...
    let lock = StoreLock::sweep(store_root, LOCK_DEADLINE)
        .map_err(|error| ScrubErrorV1::LockDenied(error.to_string()))?;
    let max_objects = config.max_objects.unwrap_or(SCRUB_MAX_OBJECTS_PER_PASS_V1);
    ...
    let mut findings: Vec<ScrubFindingV1> = Vec::new();
    ...
    let identities = cas.list_objects_bounded(max_objects).map_err(|error| match &error {
        CasError::Malformed(message) if message.starts_with("CAS object enumeration exceeds") => {
            ScrubErrorV1::EnumerationExceedsBound { max: max_objects }
        }
        _ => ScrubErrorV1::Io(error.to_string()),
    })?;
        ...
        match cas.get_verified_limited(&identity, config.max_object_bytes) {
            ...
            Err(CasError::DigestMismatch { .. }) => {
                cas.quarantine_object(&identity, &lock)
                    .map_err(|error| ScrubErrorV1::Io(error.to_string()))?;
                corrupt_quarantined += 1;
                findings.push(ScrubFindingV1 {
                    identity: identity.clone(),
                    kind: ScrubFindingKindV1::CorruptQuarantined,
            ...
            Err(CasError::NotFound) => {
                ...
                findings.push(ScrubFindingV1 {
                    identity,
                    kind: ScrubFindingKindV1::Unavailable,
            ...
            Err(error) => {
                ...
                findings.push(ScrubFindingV1 {
                    identity,
                    kind: ScrubFindingKindV1::IoError,
    ...
    let receipt = ScrubReceiptV1 {
        schema_version: SCRUB_SCHEMA_VERSION_V1,
...
pub fn read_scrub_receipt_v1(
    ...
    let bytes = fs::read(&path).map_err(|error| ScrubErrorV1::Io(error.to_string()))?;
    let value: serde_json::Value =
        serde_json::from_slice(&bytes).map_err(|error| {
            ScrubErrorV1::SchemaViolation(format!("receipt decode failed: {error}"))
        })?;
    ...
    let receipt: ScrubReceiptV1 = serde_json::from_value(value).map_err(|error| {
        ScrubErrorV1::SchemaViolation(format!("receipt structure failed: {error}"))
    })?;
    receipt.validate()?;
    Ok(receipt)
}
