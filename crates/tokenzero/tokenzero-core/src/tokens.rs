...
/// `estimate:` looks labeled but is the Phase-4 honesty hole: kernel used
/// to emit it and Pulse never accepted it. It is not an alias of `estimator:`.
...
/// Fail-closed honesty preflight for kernel/Pulse tokenizer ids. Full Pulse grammar (`estimator:` /
/// `tiktoken:` / `provider/model@hex`) stays in Pulse. This gate is the retry predicate: unlabeled
/// `estimate:` is never an estimator alias, and `Q99` / `exact` never parse as exact identities.
...
/// Loss declaration emitted when a visible budget drops bytes without a recovery ref.
/// All emitters use this exact value.
pub const VISIBLE_BUDGET_LOSSY_DECLARATION: &str = "[mode=lossy lossy_policy_id=tokenzero.visible-compression lossy_spans=[{description=omitted-bytes reason=visible-budget recovery_may_be_needed=true}]]";
...
/// Tokenizer families whose local token-cost characteristics TokenZero knows. No
/// tokenizer vocabulary is linked today. The registered families therefore use
/// disclosed average character costs; unknown models retain the legacy lexical counter exactly.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
static ACTIVE_TOKENIZER: LazyLock<ActiveTokenizer> = LazyLock::new(|| {
    let model_id = ["TOKENZERO_MODEL", "OPENAI_MODEL"]
        .into_iter()
...
/// Model selected once from `TOKENZERO_MODEL`, then `OPENAI_MODEL`, in precedence order.
...
/// Per-byte classification for the ASCII fast path of `count_tokens`. 0 =
/// non-whitespace separator (counts as a token if not currently in a token) 1 =
/// in-token (alphanumeric or `_`) 2 = whitespace (breaks tokens, never itself a token)
#[rustfmt::skip]
...
/// Tokenizer id for [`count_tokens`] and [`super::Accounting`] JSON. This is always a labeled
/// kernel estimator family, never bare `estimate:`, `Q99`, or an invented exact identity.
...
/// Pulse and capsule accounting preserve the signed cost.
