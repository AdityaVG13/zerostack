...
pub enum TypedQuery {
    ReadSpan { object: String, start: u64, end: u64 },
    ExactSearch { scope: String, pattern: Vec<u8> },
    Definition { symbol: u64 },
    References { symbol: u64 },
    AstClosure { seeds: Vec<u64>, relations: u64, radius: u32 },
    CallPath { source: u64, target: u64 },
    DataflowSlice { sink: u64 },
    Diff { old: String, new: String },
    BuildReceipt { command: u64 },
    TestTrace { test: u64 },
}
...
pub enum CompletenessWitness {
    ReadSpan,
    ExactSearch { scope: String, pattern: Vec<u8>, scope_len: u64, match_count: u64 },
    Definition { symbol: u64, index_id: String, index_version: String },
    References { symbol: u64, index_id: String, index_version: String, match_count: u64 },
    AstClosure { seeds: Vec<u64>, relations: u64, radius: u32, parser_id: String, parser_version: String, visited_nodes: u64 },
    CallPath { source: u64, target: u64, edge_count: u64 },
    DataflowSlice { sink: u64, visited_nodes: u64 },
    Diff { old: String, new: String },
    BuildReceipt { command: u64, exit_code: i32, stdout_digest: String, stderr_digest: String },
    TestTrace { test: u64, exit_code: i32, trace_digest: String },
}
...
pub fn immutable_query_fixtures() -> Vec<QueryFixture> {
    ...
    vec![
        QueryFixture { query: TypedQuery::ReadSpan { object: "fixture-v1".into(), start: 0, end: 5 }, source: SOURCE },
        QueryFixture { query: TypedQuery::ExactSearch { scope: "fixture-v1".into(), pattern: b"alpha".to_vec() }, source: SOURCE },
        QueryFixture { query: TypedQuery::Definition { symbol: 7 }, source: SOURCE },
        QueryFixture { query: TypedQuery::References { symbol: 7 }, source: SOURCE },
        QueryFixture { query: TypedQuery::AstClosure { seeds: vec![1, 2], relations: 3, radius: 2 }, source: SOURCE },
        QueryFixture { query: TypedQuery::CallPath { source: 7, target: 9 }, source: SOURCE },
        QueryFixture { query: TypedQuery::DataflowSlice { sink: 4 }, source: SOURCE },
        QueryFixture { query: TypedQuery::Diff { old: "fixture-v0".into(), new: "fixture-v1".into() }, source: SOURCE },
        QueryFixture { query: TypedQuery::BuildReceipt { command: 11 }, source: SOURCE },
        QueryFixture { query: TypedQuery::TestTrace { test: 13 }, source: SOURCE },
    ]
}
...
fn independently_derive_certificate(fixture: &QueryFixture) -> RaccCertificate {
    ...
    let (payload, completeness) = match &query {
        ...
        TypedQuery::ExactSearch { scope, pattern } => {
            ...
            (vec![pattern.clone(); matches as usize], CompletenessWitness::ExactSearch {
                scope: scope.clone(), pattern: pattern.clone(), scope_len: fixture.source.len() as u64, match_count: matches,
            })
        }
        TypedQuery::Definition { symbol } => (vec![b"fn target".to_vec()], CompletenessWitness::Definition {
            symbol: *symbol, index_id: "fixture-index".into(), index_version: "4".into(),
        }),
        TypedQuery::References { symbol } => (vec![b"target()".to_vec(), b"target".to_vec()], CompletenessWitness::References {
            symbol: *symbol, index_id: "fixture-index".into(), index_version: "4".into(), match_count: 2,
        }),
        TypedQuery::AstClosure { seeds, relations, radius } => (vec![b"1,2,3,4".to_vec()], CompletenessWitness::AstClosure {
            seeds: seeds.clone(), relations: *relations, radius: *radius, parser_id: "fixture-parser".into(), parser_version: "1".into(), visited_nodes: 4,
        }),
        TypedQuery::CallPath { source, target } => (vec![b"7>8>9".to_vec()], CompletenessWitness::CallPath { source: *source, target: *target, edge_count: 2 }),
        TypedQuery::DataflowSlice { sink } => (vec![b"1>3>4".to_vec()], CompletenessWitness::DataflowSlice { sink: *sink, visited_nodes: 3 }),
        TypedQuery::Diff { old, new } => (vec![b"+ target".to_vec()], CompletenessWitness::Diff { old: old.clone(), new: new.clone() }),
        TypedQuery::BuildReceipt { command } => (vec![b"exit=0".to_vec()], CompletenessWitness::BuildReceipt {
            command: *command, exit_code: 0, stdout_digest: digest_hex(b"built"), stderr_digest: digest_hex(b""),
        }),
        TypedQuery::TestTrace { test } => (vec![b"pass".to_vec()], CompletenessWitness::TestTrace {
            test: *test, exit_code: 0, trace_digest: digest_hex(b"trace-13"),
        }),
    };
    RaccCertificate { schema_version: 1, domain: "zerostack.racc.typed-query.v1".into(), query, payload, provenance: locked_provenance(), completeness }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BudgetMutation { None, Nonnested, UnderreportedCost }
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ResidencyMutation { None, Corruption, SilentMiss }
...
impl Charges {
    pub fn total(&self) -> u64 {
        self.successful_trials + self.failed_trials + self.retries + self.verification_calls
            + self.recovery_calls + self.expansions + self.failed_expansions + self.fallback_charges
    }
}
...
pub fn canonical_replay_identity(target_identity: &str, target_digest: &str, phases: &[PhaseArithmetic]) -> String {
    ...
    for phase in phases {
        ...
        for value in [phase.charges.successful_trials, phase.charges.failed_trials, phase.charges.retries,
            phase.charges.verification_calls, phase.charges.recovery_calls, phase.charges.expansions, phase.charges.failed_expansions,
            phase.charges.fallback_charges, phase.reported_total] {
...
pub enum IrreversibleDecision { RawFallback, CommittedCompressed }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InlineObservation { pub payload: Vec<u8>, pub certificate: RaccCertificate, pub round_trips: u32 }

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StoredFixture { pub id: String, pub bytes: Vec<u8>, pub metadata: BTreeMap<String, String> }
#[derive(Clone, Debug, Eq, PartialEq)]
...
fn fail(id: RaccGateId, detail: impl Into<String>) -> RaccCheckResult { RaccCheckResult { id, status: CheckStatus::Fail, detail: detail.into() } }
...
pub fn check_cert(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    for fixture in immutable_query_fixtures() {
        ...
        if actual != expected {
            return fail(RaccGateId::Cert, format!("independent payload/provenance/completeness derivation rejected {:?}", fixture.query));
        }
    }
    pass(RaccGateId::Cert, "all ten supported typed query kinds independently derived")
}
...
pub fn check_receipt(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    ...
    if receipt.schema_version != 1 || receipt.target_identity != "fixture-target" || receipt.target_digest != digest_hex(b"fixture-target-v1") {
        return fail(RaccGateId::Receipt, "target identity or digest changed");
    }
    if receipt.phases != expected { return fail(RaccGateId::Receipt, "exact phase charge arithmetic or complete accounting mismatch"); }
    if receipt.phases.iter().any(|phase| phase.reported_total != phase.charges.total()) {
        return fail(RaccGateId::Receipt, "reported phase total does not equal independently summed charges");
    }
    let replay = canonical_replay_identity(&receipt.target_identity, &receipt.target_digest, &receipt.phases);
    if receipt.replay_identity != replay { return fail(RaccGateId::Receipt, "replay identity mismatch"); }
    pass(RaccGateId::Receipt, "replay identity and every charge dimension recomputed")
}
...
pub fn check_irreversible_gate(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    match substrate.irreversible_without_evidence() {
        IrreversibleDecision::RawFallback => pass(RaccGateId::GateIrrev, "unproven irreversible effect routed to RawFallback"),
        IrreversibleDecision::CommittedCompressed => fail(RaccGateId::GateIrrev, "irreversible gate was skipped"),
    }
}

pub fn check_budget(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    let observation = substrate.expansion_budget();
    if observation.requested.is_empty() || observation.requested.len() != observation.measured_costs.len() || observation.measured_costs != observation.reported_costs {
        return fail(RaccGateId::Budget, "measured and reported expansion cost mismatch");
    }
    if observation.requested.windows(2).any(|pair| pair[1] != pair[0].saturating_mul(2)) {
        return fail(RaccGateId::Budget, "budgets are not nested monotone doublings");
    }
    if observation.measured_costs.iter().zip(&observation.requested).any(|(cost, budget)| cost > budget) {
        return fail(RaccGateId::Budget, "an expansion exceeded its enclosing budget");
    }
    let cumulative = observation.measured_costs.iter().try_fold(0_u64, |sum, cost| sum.checked_add(*cost));
    let factor_four = observation.requested.last().and_then(|last| last.checked_mul(4));
    ...
    pass(RaccGateId::Budget, "nested doubling and factor-4 arithmetic verified")
}
...
pub fn check_inline(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    ...
    let expected = independently_derive_certificate(&fixture);
    if observation.round_trips != 1 { return fail(RaccGateId::Inline, "certificate required a second fetch"); }
    if observation.payload != expected.payload.concat() || observation.certificate != expected {
        return fail(RaccGateId::Inline, "inline payload/certificate pair is not independently valid");
    }
    pass(RaccGateId::Inline, "payload and certificate arrived in one round trip")
}

pub fn residency_fixtures(n: usize) -> Vec<StoredFixture> {
    (0..n).map(|index| StoredFixture {
        id: format!("resident-{index}"), bytes: format!("immutable-object-{index}\0binary").into_bytes(),
        metadata: BTreeMap::from([("content_type".into(), "application/octet-stream".into()), ("ordinal".into(), index.to_string())]),
    }).collect()
}
...
pub fn check_residency(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    ...
    if observations.len() != objects.len() { return fail(RaccGateId::Residency, "substrate omitted a residency observation"); }
    for (object, (resident, removed)) in objects.iter().zip(observations) {
        match resident {
            StoreLookup::Hit { bytes, metadata } if bytes == object.bytes && metadata == object.metadata => {}
            _ => return fail(RaccGateId::Residency, format!("resident object {} was not byte-identical with metadata", object.id)),
        }
        match removed {
            StoreLookup::Miss { id } if id == object.id => {}
            _ => return fail(RaccGateId::Residency, format!("guarded removal of {} did not yield a typed miss", object.id)),
        }
    }
...
pub fn check_task_transaction(substrate: &mut impl RaccSubstrate) -> RaccCheckResult {
    ...
    if !passing_receipt_valid(&passing, receipt) {
        return fail(RaccGateId::TaskTransaction, "passing objective verifier/receipt/charge contract mismatch");
    }
...
pub fn check_release_aggregate(evidence: &ReleaseEvidence) -> ReleaseAggregateResult {
    let ratios: Vec<_> = evidence.tasks.iter().filter_map(|task| task.ratio().map(|(c, r)| (task.task_id.clone(), c, r))).collect();
    let targets_fixed = release_targets_fixed(evidence);
    let no_regression = evidence.tasks.iter().all(|task| regression_evidence_valid(&task.evidence));
    ...
    } else if valid { "paper 12.2 release aggregate passed" } else { "paper 12.2 release aggregate failed" };
    ReleaseAggregateResult { status: if valid { CheckStatus::Pass } else { CheckStatus::Fail }, production_release_pass, task_ratios: ratios, detail: detail.into() }
}

pub fn digest_hex(bytes: &[u8]) -> String {
    let mut lanes = [0xcbf29ce484222325_u64, 0x84222325cbf29ce4, 0x9e3779b185ebca87, 0x517cc1b727220a95];
...
pub fn validate_racc_schema(schema: &str, document: &Value) -> Result<(), String> {
    ...
    validator.validate(document).map_err(|error| error.to_string())
}
