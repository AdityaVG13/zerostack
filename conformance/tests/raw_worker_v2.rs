...
fn engine_identities_are_closed_and_canonical() {
    for canonical in ["fszero", "graphzero", "tokenzero"] {
        ...
        assert_eq!(serde_json::to_value(request).unwrap()["approval_grant"]["engine"], canonical);
    }
    assert!(serde_json::from_value::<CallRequest>(serde_json::to_value(approval_call("fszero")).unwrap()).is_ok());
...
fn approval_grant_binding_expiry_effect_digest_and_replay_are_enforced() {
    ...
    assert!(request.validate_approval_grant(engine, "/workspace", "session", effect, 150, &mut consumed).is_ok());
    assert_eq!(format!("{:?}", request.validate_approval_grant(engine, "/workspace", "session", effect, 150, &mut consumed).unwrap_err()), "Replayed");

    let mut missing = serde_json::to_value(&request).unwrap(); missing.as_object_mut().unwrap().remove("approval_grant");
    let missing: CallRequest = serde_json::from_value(missing).unwrap();
    assert_eq!(format!("{:?}", missing.validate_approval_grant(engine, "/workspace", "session", effect, 150, &mut BTreeSet::new()).unwrap_err()), "Missing");
    assert_eq!(format!("{:?}", request.validate_approval_grant(engine, "/other", "session", effect, 150, &mut BTreeSet::new()).unwrap_err()), "BindingMismatch");
    assert_eq!(format!("{:?}", request.validate_approval_grant(engine, "/workspace", "session", effect, 200, &mut BTreeSet::new()).unwrap_err()), "Expired");

    for (field, value, expected) in [
        ("effect", json!("read_only"), "WrongEffect"),
        ("policy_digest", json!("ABC"), "Malformed"),
    ] {
        let mut value_request = serde_json::to_value(&request).unwrap(); value_request["approval_grant"][field] = value;
        let parsed: CallRequest = serde_json::from_value(value_request).unwrap();
        assert_eq!(format!("{:?}", parsed.validate_approval_grant(engine, "/workspace", "session", effect, 150, &mut BTreeSet::new()).unwrap_err()), expected);
    }
}
