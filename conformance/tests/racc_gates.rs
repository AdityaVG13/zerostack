...
fn assert_fails(result: zerostack_codemode_conformance::racc::RaccCheckResult) {
    assert_eq!(result.status, CheckStatus::Fail, "mutation unexpectedly passed: {}", result.detail);
}
...
fn certificate_gate_rejects_every_required_mutation_class() {
    for mutation in [
        ...
        let mut fake = RaccFakeSubstrate { certificate_mutation: mutation, ..Default::default() };
...
fn receipt_gate_rejects_identity_arithmetic_and_every_hidden_charge() {
    for mutation in [
        ...
        let mut fake = RaccFakeSubstrate { receipt_mutation: mutation, ..Default::default() };
...
fn irreversible_gate_rejects_a_committed_unproven_effect() {
    let mut fake = RaccFakeSubstrate { skip_irreversible_gate: true, ..Default::default() };
...
fn budget_gate_rejects_nonnested_and_underreported_expansion_cost() {
    for mutation in [BudgetMutation::Nonnested, BudgetMutation::UnderreportedCost] {
        let mut fake = RaccFakeSubstrate { budget_mutation: mutation, ..Default::default() };
...
fn inline_gate_rejects_a_second_certificate_fetch() {
    let mut fake = RaccFakeSubstrate { second_certificate_fetch: true, ..Default::default() };
...
fn residency_gate_rejects_corruption_and_silent_miss() {
    for mutation in [ResidencyMutation::Corruption, ResidencyMutation::SilentMiss] {
        let mut fake = RaccFakeSubstrate { residency_mutation: mutation, ..Default::default() };
        assert_fails(check_residency(&mut fake));
    }
}

fn release_fixture(fake: bool) -> ReleaseEvidence {
    let accounting = Charges { successful_trials: 3, failed_trials: 2, retries: 2, verification_calls: 5, recovery_calls: 2, expansions: 3, failed_expansions: 2, fallback_charges: 3 };
    ReleaseEvidence {
...
fn release_aggregate_preserves_paper_12_2_and_labels_fake_green() {
    ...
    assert_eq!(fake.task_ratios, vec![("task-paired".into(), 75, 100), ("task-t13".into(), 80, 80)]);
...
fn release_aggregate_rejects_unfixed_target_regression_and_hidden_accounting() {
    ...
    regression.tasks[0].evidence = RegressionEvidence::PoweredPaired { powered: false, no_regression: true };
    assert_eq!(check_release_aggregate(&regression).status, CheckStatus::Fail);
