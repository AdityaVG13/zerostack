...
use graphzero_reserve::{
    DeclareRequest, ReservationLedger, ReservationStatus, ReserveService, check_reservation,
    check_reservation_with_ttl, contract_footprint, declare_reservation, ledger_state_hash,
    list_active_reservations, release_reservation, replay_ledger, test_notify_conflict,
...
fn declare_acquire_release_lifecycle() {
    let fx = common::reserve_indexed_fixture();
    let ops = common::parse_ref_ops();
    ...
    assert!(decl.footprint_ref.starts_with("gz://footprint/"));
    assert_eq!(decl.status, ReservationStatus::Declared);
    assert!(decl.ttl_seconds >= 60);

    common::acquire_parse_ref(&fx, "AgentA");
...
fn conflict_declared_and_cross_agent() {
    ...
    let fx2 = common::reserve_indexed_fixture();
    common::acquire_parse_ref(&fx, "AgentA");
    ...
    for edge in conflict.conflict_edges {
        assert!(edge.evidence_ref.starts_with("gz://"));
    }
    assert_eq!(
...
fn query_active_footprint_reports_active_reservations() {
    let fx = common::reserve_indexed_fixture();
    common::acquire_parse_ref(&fx, "A1");
    common::acquire_agent(&fx, "A2", &common::load_config_ops());
    assert_eq!(
    ...
    assert!(fp.footprint_ref.starts_with("gz://footprint/"));
}
...
fn notify_hook_and_ledger_replay() {
    ...
    test_notify_conflict("AgentB", &["gz://node/parse_ref".into()]);
    assert_eq!(test_notify_hook_count(), 1);

    for i in 0..5 {
        common::acquire_parse_ref(&fx, &format!("Agent{i}"));
    }
...
fn service_reopens_snapshot_after_store_is_reindexed() {
    ...
    let first = service
        .check("AgentA", &common::parse_ref_ops(), false)
        .unwrap();
...
fn concurrent_acquire_same_symbol_allows_only_one_active_reservation() {
    let fx = Arc::new(common::reserve_indexed_fixture());
    let ops = common::parse_ref_ops();
