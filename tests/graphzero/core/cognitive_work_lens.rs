//! Wave16 Task Lens internal contract laws (graphzero-core engine domain).
//!
//! Trivalent fail-closed laws over the cognitive-work contracts:
//!
//!   - `Safe` only for a unique rooted exact locus, exact/complete reverse
//!     impact, complete coverage and freshness, and every obligation
//!     discharged by a fresh proof support bound to the requested snapshot.
//!   - `Unsafe` only for explicit semantic choice/conflict (decision gaps).
//!     Ambiguous candidate loci and underapproximated fibers are `Unknown`,
//!     never upgraded to `Unsafe`.
//!   - `Unknown` for everything missing, stale, or incomplete. Bounded
//!     reverse impact refuses to certify completeness beyond its budget.
