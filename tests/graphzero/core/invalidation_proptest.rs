//! Property-based tests for the certified invalidation engine (GRAPH-006).
//!
//! Random-DAG properties over the invalidation machinery
//! (`graphzero-core/src/invalidation.rs`):
//!
//!   1. `upward_closure_never_under_invalidates`: every artifact whose
//!      recompute differs between baseline and edited full rebuilds is inside
//!      the certified invalidated set (never under-invalidate).
//!   2. `incremental_equals_full_with_cutoff`: incremental recompute with the
//!      equality-boundary early cutoff is bit-identical to a full rebuild on
//!      random edits, with equal-value (constant) producers active so the
//!      cutoff fires (GRAPH-007).
//!   3. `cutoff_report_invariants`: `recomputed` and `cut_off` are disjoint,
//!      boundary nodes are a subset of `recomputed`, and
//!      `recomputed | cut_off` covers every dirty producer (honest savings
//!      accounting).
//!   4. `failed_producer_never_cuts_off_downstream`: with randomly failing
//!      producers, no boundary node is ever downstream of a failed producer
//!      (taint fail-closed).
//!   5. `under_invalidation_is_never_silent`: when the influence graph hides a
//!      true dependency, incremental recompute must not silently match full
//!      rebuild -- the protected equivalence check fails with
//!      `EquivalenceDivergence` instead.
//!
//! Graphs are bounded to <= 20 nodes (4 sources + up to 16 producers) and the
//! case count is pinned to 64 (`ProptestConfig::with_cases`) for CI speed.
...
/// Build an engine whose influence edges exactly mirror the producers' true
/// reads, except for `hidden` reads (undeclared true dependencies, property
/// 5). Constant producers ignore all inputs; flaky producers return None
/// while their first prerequisite carries the poison marker.
...
/// Producers that failed this pass, detected deterministically from the final
/// state: a flaky producer fails iff its first prerequisite's (final) value
/// carries the poison marker. The prerequisite index is smaller than the
/// producer's, so its value is final when the producer runs in topo order.
...
proptest! {
    ...
    /// Property 1: upward-closure soundness. On exact random DAGs, every
    /// artifact whose recompute differs between baseline and edited full
    /// rebuilds is inside the certified invalidated set -- never under-
    /// invalidate (over-invalidation is allowed for sound overapprox).
    #[test]
    ...
    /// Property 5: under-invalidation is never silent. When every producer
    /// reads an undeclared hidden dependency, incremental recompute must
    /// either match the full rebuild (hidden value unchanged / producer still
    /// reached) or the protected equivalence check must fail closed with
    /// EquivalenceDivergence -- never silently accept a stale answer.
    #[test]
    fn under_invalidation_is_never_silent(
        ...
        if incr.state == full.state {
            ...
            // A hidden (undeclared) dependency made incremental diverge from
            // the full rebuild -- whether by escaping the certified set or by
            // an equality cutoff that a complete graph would have forbidden.
            // Either way the protected check must fail closed: stale answers
