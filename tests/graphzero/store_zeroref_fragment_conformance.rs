//! ZeroRef fragment conformance through `ExpandResolver`.
//! Includes Unicode, byte-boundary, and integer-overflow cases.
...
struct Vector {
    ...
    /// only exercised by the direct ZeroRef selector with an explicit policy.
    #[serde(default)]
...
fn load_fixture() -> Fixture {
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../contracts/zeroref-fixtures.json");
...
/// Every canonical blob vector must behave identically through `ExpandResolver`:
/// the same bytes on success and the same error class on failure.
#[test]
fn resolver_expansion_matches_golden_vectors() {
    ...
    for v in &fixture.vectors {
        if v.parse != "ok" || !v.input.starts_with("z://blob/") {
            continue;
        }
        ...
        exercised += 1;
        let gz = GzRef::parse(&v.input).expect("engine grammar accepts canonical blob refs");
...
/// Cases beyond the shared fixture: Unicode multi-byte content, first/last byte boundaries, and
/// overflow spellings. Each runs through BOTH the standalone selector and the resolver so the
...
fn local_case_matrix_covers_unicode_boundaries_and_overflow() {
    ...
    for case in &cases {
        ...
        let reference = format!("z://blob/{hash}{}", case.fragment);

        // Compare the portable parser with the domain resolver.
        let direct_outcome = ZeroRef::parse(&reference)
        ...
        match case.expect {
            Ok(expected) => {
                let direct =
                    direct_outcome.unwrap_or_else(|e| panic!("case '{}' direct: {e}", case.name));
                let res = resolver_outcome
                    .unwrap_or_else(|e| panic!("case '{}' resolver: {e}", case.name));
                assert_eq!(direct, expected, "case '{}' direct bytes", case.name);
                assert_eq!(res, expected, "case '{}' resolver bytes", case.name);
            }
            Err(class) => {
                let direct_err =
                    direct_outcome.expect_err(&format!("case '{}' direct must fail", case.name));
                assert_eq!(
                    direct_err.class.as_str(),
                    class,
                    "case '{}' direct class",
                    case.name
                );
                ...
                // Resolver parse errors are untyped strings; range failures preserve
                // the stable class while malformed spellings need only fail closed.
                assert!(
                    res_err.contains(class) || (class == "malformed" && !res_err.is_empty()),
...
fn valid_window_cannot_hide_corruption_outside_it() {
    ...
    .unwrap();
    let resolver = ExpandResolver::new(dir.path(), None).unwrap();
    let reference = format!("z://blob/{hash}#B0-11");
...
#[test]
fn graph_ref_parser_rejects_retired_aliases() {
    ...
    for reference in [
        ...
        format!("tz://blob/{hash}"),
        format!("blob/{hash}"),
        format!("z://blob/{hash}#B0+1"),
    ] {
        assert!(
            GzRef::parse(&reference).is_err(),
            "retired alias parsed: {reference}"
        );
    }
}

#[test]
fn graph_store_helpers_reject_retired_aliases() {
    assert_eq!(
        graphzero_store::path_from_evidence_ref("path/src/lib.rs").as_deref(),
        Some("src/lib.rs")
    );
    assert!(graphzero_store::path_from_evidence_ref("gz://path/src/lib.rs").is_none());
    let hash = "a".repeat(64);
    assert_eq!(
        graphzero_store::blob_hash_from_ref(&format!("z://blob/{hash}")),
        Some(hash.clone())
    );
    assert!(graphzero_store::blob_hash_from_ref(&format!("gz://b/{hash}")).is_none());
}
