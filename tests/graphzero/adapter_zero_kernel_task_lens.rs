//! root tests over a real indexed fixture (graphzero-engine `StructuralEngine::task_lens`).
...
use zero_abi::{
    ...
    KernelBudget, SafetyVerdict, StructuralEngine, StructuralQuery, TaskLensRequest, ZeroHandle,
};
...
/// Fixture layout `lens_build` is called by `lens_consume` (one reverse calls edge);
/// `lens_ambig` is defined twice (ambiguous query); `LensTarget` is a plain struct with no callers.
...
#[test]
fn natural_query_refreshes_stale_graph_before_claiming_complete() {
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("repo");
    fixture_repo(&repo);
    let graph_store = temp.path().join("graph");
    ...
    let query = || StructuralQuery {
        query: "lens_build".into(),
        options: lens_options(AsgrepMode::Natural),
    };
    let initial = engine.query(&invocation(&repo), query()).unwrap();
    assert!(
        initial
            .hits
            .iter()
            .any(|hit| hit.symbol.as_deref() == Some("lens_build")),
        "fixture must resolve through the natural intent anchor"
    );
    ...
    let expected_line = shifted
        .lines()
        .position(|line| line.contains("fn lens_build"))
        .map(|line| u32::try_from(line + 1).unwrap())
        .unwrap();

    let refreshed = engine.query(&invocation(&repo), query()).unwrap();
    let hit = refreshed
        .hits
        .iter()
        .find(|hit| hit.symbol.as_deref() == Some("lens_build"))
        .expect("refreshed graph must retain the definition");
    assert_eq!(
        hit.line_start,
        Some(expected_line),
        "natural query must not pair a stale line with a current source handle"
    );
    let coverage = refreshed
        .coverage
        .expect("natural query must report coverage");
    assert!(coverage.freshness_verified);
    assert_ne!(coverage.snapshot_id, 0);
}
