...
fn set_index(path: &std::path::Path) -> graphzero_test_support::ScopedEnvVars {
    let mut env = graphzero_test_support::ScopedEnvVars::new();
...
fn disable_index(path: &std::path::Path) -> graphzero_test_support::ScopedEnvVars {
    let mut env = graphzero_test_support::ScopedEnvVars::new();
    env.set("GRAPHZERO_REF_INDEX_PATH", path);
...
fn unauthorized_indexed_root_emits_wrong_root() {
    ...
    let reference = format!("gz://query/{query_id}");
...
fn blob_put_fails_when_ref_index_cannot_be_recorded() {
    ...
    let message = format!("{err:#}");
    assert!(
        message.contains("record ref-index entry for gz://blob/"),
        "{message}"
    );
}
...
/// Bead 1ghi.5: the index must work when its directory path contains spaces
...
fn index_works_under_unicode_and_spaced_paths() {
    ...
    let found = ref_index::lookup_store(&format!("gz://blob/{hash}"));
    assert_eq!(
...
/// Bead 1ghi.5: a leftover compaction temp (simulated interrupted replace)
/// must never shadow or corrupt the shard — readers see the old valid index,
/// and the next compaction still publishes a complete file.
#[test]
#[serial]
fn interrupted_replace_leaves_a_valid_index() {
    ...
    let hash = blob_store
        ...
        .unwrap()
        .to_hex();
    let ref_id = format!("gz://blob/{hash}");
    ...
    fs::write(&torn, b"{\"ref_id\":\"gz://blob/tor").unwrap();
