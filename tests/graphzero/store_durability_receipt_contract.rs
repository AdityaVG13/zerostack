...
use zero_abi::{DurableProfileId, Sha256Digest, canonical_json};
use zero_store::{JournalBoundary, JournalError, JournalFailureCode};
...
fn evidence_input() -> DurabilityEvidenceInput {
    ...
    let surface = CanonicalSurfaceBytes {
        ...
        ref_bytes: canonical_json(&json!([format!("z://blob/{}", hash.to_hex())])).into_bytes(),
    ...
    let metadata = DurabilityMetadata {
        ...
        fixture_identity: "durability-receipt-fixture".into(),
        verifier_identity: "graphzero-durability-verifier".into(),
