...
use zero_abi::{
    ...
    savings_overhead_killed, sha256_hex,
};
...
fn causal_positive_fixture_is_safe() {
    ...
    // Independently computed evidence: one item per unique demanded output,
    // canonical sorted order, each digest binds the demanded id.
    let mut sorted = demanded.to_vec();
    sorted.sort_unstable();
    let expected: Vec<(String, String)> = sorted
        .iter()
        .enumerate()
        .map(|(idx, id)| (format!("demand:{idx}"), sha256_hex(id.as_bytes())))
        .collect();
    assert_eq!(report.evidence.items.len(), expected.len());
    for (idx, (exp_name, exp_digest)) in expected.iter().enumerate() {
        assert_eq!(&report.evidence.items[idx].name, exp_name);
        assert_eq!(&report.evidence.items[idx].digest, exp_digest);
    }
    assert_eq!(report.evidence.anchor, sha256_hex(&bytes));
...
fn causal_duplicate_demands_dedupe() {
    let bytes_dup = causal_bytes(
        &["d1", "d1", "d2"],
        &[("d1", "declared_output"), ("d2", "declared_output")],
        &[],
    );
    let bytes_unique = causal_bytes(
        &["d1", "d2"],
        &[("d1", "declared_output"), ("d2", "declared_output")],
        &[],
    );
    let dup = check_causal_closure(&bytes_dup).unwrap();
    let uniq = check_causal_closure(&bytes_unique).unwrap();
    assert_eq!(dup.verdict, SafetyVerdict::Safe);
    assert_eq!(uniq.verdict, SafetyVerdict::Safe);
    // Duplicate input normalizes to unique-demand evidence.
    assert_eq!(dup.evidence.items, uniq.evidence.items);
    for id in ["d1", "d2"] {
        ...
        let count = dup
            ...
            .count();
        assert_eq!(count, 1, "demand {id} should appear exactly once");
    }
    assert_eq!(dup.evidence.items.len(), 2);
}
#[test]
...
fn kill_non_converging_counterexamples() {
    let mut metrics = KillMetrics::new();
    // First root: through MAX_ISSUES no kill.
    ...
    // Next observation records exactly one kill for that root.
    metrics.observe_counterexample(ROOT_A);
    assert_eq!(metrics.non_converging_counterexamples(), 1);
    // Further observations for the same root do not increment again.
    metrics.observe_counterexample(ROOT_A);
    metrics.observe_counterexample(ROOT_A);
    assert_eq!(metrics.non_converging_counterexamples(), 1);

    // Second root is independent: through MAX_ISSUES still no additional kill.
    for _ in 0..VCQK_KILL_NONCONVERGENCE_MAX_ISSUES {
...
fn learning_and_refinement_have_no_publish_authority() {
    let mut metrics = KillMetrics::new();
    // Seed with a Safe report to establish a tracked certificate root.
    let report =
        check_causal_closure(&causal_bytes(&["d1"], &[("d1", "declared_output")], &[])).unwrap();
    assert!(report.grants_authority());
    let root = report.certificate.as_ref().unwrap().root.clone();
    ...
    // Drive counterexample processing through the kill threshold.
    for _ in 0..VCQK_KILL_NONCONVERGENCE_MAX_ISSUES {
        metrics.observe_counterexample(&root);
        assert_eq!(metrics.learning_publications(), 0);
        assert_eq!(metrics.non_converging_counterexamples(), 0);
    }
    metrics.observe_counterexample(&root);
    assert_eq!(metrics.non_converging_counterexamples(), 1);
    assert_eq!(metrics.learning_publications(), 0);
    // Further reissues still do not publish.
    metrics.observe_counterexample(&root);
    assert_eq!(metrics.learning_publications(), 0);

    // Refutation of the Safe root is false authority, not a publication.
    metrics.observe_refutation(&root);
    assert_eq!(metrics.false_authority(), 1);
    assert_eq!(metrics.learning_publications(), 0);

    // Savings overhead path also never publishes.
    metrics.observe_savings(5, 10);
    assert_eq!(metrics.savings_overhead(), 1);
    assert_eq!(metrics.learning_publications(), 0);
    metrics.observe_savings(0, 1);
    ...
    // Second independent counterexample root: still no publication through threshold.
    let other = sha256_hex(b"other-counterexample");
    for _ in 0..VCQK_KILL_NONCONVERGENCE_MAX_ISSUES {
        metrics.observe_counterexample(&other);
        assert_eq!(metrics.learning_publications(), 0);
    }
    metrics.observe_counterexample(&other);
    assert_eq!(metrics.non_converging_counterexamples(), 2);
    ...
    // No checker output from learning inputs grants authority or carries a certificate.
    let learning_bytes = causal_bytes(&[other.as_str()], &[(other.as_str(), "derived")], &[]);
    let learning_report = check_causal_closure(learning_bytes.as_slice()).unwrap();
    // This input is Unsafe (demanded outside closure) or Safe only if closed;
    // in neither case does the refinement path mint a publication certificate.
    if learning_report.grants_authority() {
        assert!(learning_report.certificate.is_some());
    } else {
        assert!(learning_report.certificate.is_none());
        let text = String::from_utf8(learning_report.to_canonical_bytes().unwrap()).unwrap();
