//! FileEngine capsule adapter. `put_capsule` stores a WorkCapsule and returns its exact
//! publication handle. `get_capsule` verifies object and capsule-root hashes before use.
...
#[test]
fn line_range_past_eof_clamps_to_last_line() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("lines.txt"), b"alpha\nbeta\ngamma\n").unwrap();
    let engine = engine(&dir);

    let snapshot = engine
        .read(
            &invocation(dir.path()),
            zero_abi::FileReadRequest {
                path: "lines.txt".into(),
                options: zero_abi::ReadOptions {
                    range: Some("2:120".into()),
                    ..zero_abi::ReadOptions::default()
                },
            },
        )
        .expect("clamped line range");

    assert_eq!(snapshot.inline_utf8.as_deref(), Some("beta\ngamma\n"));
}
