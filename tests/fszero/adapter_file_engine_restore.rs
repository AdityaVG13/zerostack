//! FileEngine Restore is CAS replay, not another Write. `apply(Restore)` with no
//! content must materialize `expected_preimage` (or a prior receipt's `before`
//! handle) from CAS. Missing/wrong handles fail closed; a cancelled invocation must not mutate.
...
fn restore_recovers_bytes_mode_and_modified_time() {
    ...
    let snapshot = read_snapshot(&engine, dir.path());
    let receipt = engine
        .apply(
            &invocation(dir.path()),
            FileEffectRequest {
    ...
    engine.restore(&invocation(dir.path()), &receipt).unwrap();

    let restored = fs::metadata(&target).unwrap();
    assert_eq!(fs::read(&target).unwrap(), ORIGINAL.as_bytes());
    assert_eq!(restored.permissions().mode(), original.permissions().mode());
...
fn restore_fails_when_preexisting_parent_disappears() {
    ...
    let engine = ZeroFileEngine::open(
        dir.path(),
        dir.path().join(".zerostack"),
        "file-engine-restore-parent-contract",
    )
    .unwrap();
    let snapshot = engine
        .read(
    ...
    let error = engine
        .restore(&invocation(dir.path()), &receipt)
        .unwrap_err();
