...
use zero_abi::{
    ...
    EngineStageSpan, EngineStageTimeline, HandshakeAck, ProtocolLimits,
    RAW_WORKER_PROTOCOL_VERSION, RefOwnership, RevertMetadata, WorkerBinding, WorkerCapabilities,
    WorkerError, WorkerResponseFrame, WorkerResult, WorkerResultMetadata, WorkerTokenAccounting,
    WorkerTokenCountKind, WorkerTrace, decode_response_frame, encode_frame,
};
...
const CANCEL_ACK_GOLDEN: &str =
    r#"{"kind":"cancel_ack","request_id":"request-1","cancelled":true}"#;
const CANCEL_ACK_GOLDEN_NL: &str = concat!(
    r#"{"kind":"cancel_ack","request_id":"request-1","cancelled":true}"#,
    "\n"
);
...
fn assert_golden(frame: &WorkerResponseFrame, expected: &str) {
    ...
    // Round-trip
    let decoded = decode_response_frame(encoded_str.as_bytes(), DEFAULT_MAX_FRAME_BYTES)
        .expect("decode golden");
    assert_eq!(&decoded, frame);
    // Also decode the expected literal directly
    let decoded2 = decode_response_frame(expected.as_bytes(), DEFAULT_MAX_FRAME_BYTES)
        .expect("decode literal");
...
fn handshake_ack_is_golden() {
    ...
    let res = decode_response_frame(tampered.as_bytes(), DEFAULT_MAX_FRAME_BYTES)
        .expect("tampered still parses");
    assert_ne!(res, frame);
}
...
fn result_is_golden_and_omits_optional_fields() {
    ...
    assert!(
        v.get("engine_timeline").is_none(),
        "default Result must omit engine_timeline at top-level"
    );
    assert!(
        v.get("worker_token_accounting").is_none(),
        "default Result must omit worker_token_accounting at top-level"
    );
    // Also ensure nested metadata does not invent those keys
    let result_obj = v
        .get("result")
        .and_then(|r| r.as_object())
        .expect("result object");
    assert!(!result_obj.contains_key("engine_timeline"));
    assert!(!result_obj.contains_key("worker_token_accounting"));
}
...
fn result_with_extras_is_golden_and_includes_optional_fields() {
    ...
    let tl = v
        .get("engine_timeline")
        .expect("engine_timeline present at top-level");
    assert_eq!(tl.get("total_ns").and_then(|x| x.as_u64()), Some(1_000));
    let spans = tl
        .get("spans")
    ...
    let wta = v
        .get("worker_token_accounting")
        .expect("worker_token_accounting present at top-level");
    assert_eq!(
        wta.get("tokenizer_id").and_then(|x| x.as_str()),
        Some("test-tokenizer")
    );
    assert_eq!(
        wta.get("count_kind").and_then(|x| x.as_str()),
        Some("exact")
    );
    assert_eq!(wta.get("raw_tokens").and_then(|x| x.as_u64()), Some(100));
}
...
fn error_is_golden_and_omits_optional_fields_by_default() {
    ...
    let encoded = encode_frame(&frame, DEFAULT_MAX_FRAME_BYTES).unwrap();
    let v: serde_json::Value = serde_json::from_slice(&encoded).unwrap();
    assert!(v.get("engine_timeline").is_none());
    assert!(v.get("worker_token_accounting").is_none());
    // error.kind closed set: unknown kind must be rejected
    let bad = r#"{"kind":"error","request_id":"request-1","error":{"kind":"potato","message":"x","retryable":false},"trace":{"runtime_id":"runtime-1","cell_id":"cell-1","request_id":"request-1","trace_id":"trace-1","worker_revision":"fixture-revision","contract_digest":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}}"#;
...
fn shutdown_ack_is_golden_and_rejects_unknown_fields() {
    ...
    assert!(
        res.is_err(),
        "shutdown_ack with extra field must be rejected, got {:?}",
        res
    );
