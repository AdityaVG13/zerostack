...
use zero_abi::{CompletenessGrade, Sha256Digest, sha256};
use zero_gate::project_image::{
    DemandScenario, ExactObject, PerObjectLayers, ProjectImageManifest,
};
use zero_gate::snap_to_file::{SnapOutcome, SnapOutcomeKind, SnapPacket, SnapToFileRoute};
use zero_gate::{
    CoverageAtom, DemandRequest, GraphZeroCompletenessInput, IncrementalDeltaRequest,
...
fn layer(atom: Sha256Digest, l2: Option<bool>, l3: Option<bool>) -> PerObjectLayers {
    // helper maps the two trivalent layer flags into the 5-field PerObjectLayers
...
fn manifest(
    ...
    use zero_gate::project_image::{CausalGraphRef, ProofGraphRef, ShadowResourceLedger};
...
fn live_for(
    route: &SnapToFileRoute,
    handle: &zero_abi::SafeExpandHandle,
) -> zero_abi::LiveExpandState {
    route
        .current_live_state(handle, zero_abi::SafetyVerdict::Safe, false)
        .unwrap()
}

fn assert_reason_contains(packet: &SnapPacket, needle: &str) {
    assert!(
        packet.reasons.iter().any(|r| r.contains(needle)),
        "packet reasons {:?} must contain {:?}",
        packet.reasons,
        needle
    );
}
...
fn assert_snapped(
    ...
    SnapPacket,
    zero_abi::DecisionView,
    zero_gate::FirstExpansion,
    zero_abi::SafeExpandHandle,
) {
    match outcome {
        SnapOutcome::Snapped {
            packet,
...
fn assert_no_discovery_rows(expansion: &zero_gate::FirstExpansion) {
    for row in &expansion.ledger.rows {
        assert!(
            FIRST_EXPANSION_LEDGER_CLASSES.contains(&row.class.as_str()),
            "unexpected ledger class {:?} — discovery row must not exist",
            row.class
        );
    }
}
...
fn corpus_safe_multi_file_snap_single_expansion_no_discovery() {
    ...
    // Independently derived expected closure and byte totals (not from production helpers)
    let expected_atoms = sorted(&all);
    let returned: Vec<Sha256Digest> = expansion.atoms.iter().map(|atom| atom.atom_root).collect();
    assert_eq!(
        returned, expected_atoms,
        "must return exactly the requested atoms"
    );

    let expected_visible: u64 = 100 + 200 + 300;
    assert_eq!(expansion.visible_bytes, expected_visible);
    assert_eq!(expansion.atoms.len(), 3);
    ...
    let expected_projection_root = {
        ...
        let canonical = zero_abi::canonical_json(&json!({ "atoms": hex_sorted }));
    ...
    // Metrics reflect independently specified ground truth: visible bytes, savings
    ...
    // Zero false-complete against adjudicated ground truth
    let ground_truth: BTreeSet<Sha256Digest> = [a, b, c].into_iter().collect();
    let adjudicated = zero_gate::adjudicate(&expansion, &ground_truth);
...
fn corpus_safe_multi_file_packet_view_binding_and_ledger_reconciliation() {
    // Exercises the same safe path but asserts the remaining binding invariants
    // separately to keep failure localization precise.
    let a = digest(1);
    let b = digest(2);
    let c = digest(3);
    let all = [a, b, c];
    ...
    // Packet carries the view root and validates against the expansion's permit
    assert_eq!(packet.decision_view_root, view.root());
    let expected_proj_hex = expansion.permit.projection_root().to_hex();
    assert_eq!(
        packet.projection_root.as_deref(),
        Some(expected_proj_hex.as_str())
    );
    assert!(packet.certificate_root.is_some());
    let present: BTreeSet<String> = zero_gate::snap_to_file::snap_evidence_classes();
    assert_eq!(
        view.certificate(&zero_gate::snap_to_file::snap_evidence_classes(), &present)
            .unwrap(),
        CompletenessGrade::Proved
    );
    // Ledger totals reconcile: visible + savings == baseline
    let metrics = packet.metrics.unwrap();
    assert_eq!(
        metrics.visible_bytes as u128 + metrics.native_savings_bytes as u128,
        metrics.native_baseline_bytes as u128
    );
}
...
fn corpus_s0_exact_file_root_not_sold_as_multi_file() {
    ...
    assert!(
        ground_truth.is_subset(&expanded),
        "closure must cover ground truth"
    );
...
fn corpus_packet_and_permit_have_read_only_authority_surface() {
    ...
    // Packets and permits grant only read/expansion authority.
    // Serialization must not introduce authority and injected authority must be rejected.
    let packet_json = packet.canonical_render_json();
    let packet_value: Value = serde_json::from_str(&packet_json).expect("packet json");
    let permit_json = serde_json::to_string(&expansion.permit).expect("permit json");
    let permit_value: Value = serde_json::from_str(&permit_json).expect("permit json");

    // Injected mutation/commit fields must be rejected via deny_unknown_fields.
    for field in ["commit", "write", "transaction", "edit", "effect"] {
        let mut injected = packet_value.clone();
        injected[field] = json!("injected");
        let err = serde_json::from_value::<SnapPacket>(injected);
        assert!(
            err.is_err(),
            "packet must reject injected authority field {field:?}, got {err:?}"
        );
        let mut injected_permit = permit_value.clone();
        injected_permit[field] = json!("injected");
        let err = serde_json::from_value::<zero_abi::ExpandPermit>(injected_permit);
        assert!(
            err.is_err(),
            "permit must reject injected authority field {field:?}, got {err:?}"
        );
    }

    // Round-trip must be byte-identical and must not amplify authority.
    let reparsed: SnapPacket = serde_json::from_str(&packet_json).expect("re-parse packet");
    assert_eq!(reparsed.canonical_render_json(), packet_json);
    reparsed.validate().unwrap();
    let repermit: zero_abi::ExpandPermit =
        serde_json::from_str(&permit_json).expect("re-parse permit");
    assert_eq!(
        serde_json::to_string(&repermit).expect("re-serialize permit"),
        permit_json
    );

    // Attempt to use the permit for a protected-scope mutation must be rejected via revalidation.
    // Alter the live protected scope and ensure revalidation fails.
    let mut tampered_live = live_for(&gate, &_handle);
    tampered_live.protected_scope_root = digest(0x99);
    let outcome = gate.revalidate(&_handle, &tampered_live);
    assert!(
        !matches!(outcome, zero_abi::ExpandOutcome::Safe(_)),
        "tampered protected scope must not grant authority, got {outcome:?}"
    );
}
...
fn corpus_decision_view_certificate_laws_hold() {
    ...
    let mut gate = route();
    let (packet, view, _, _) =
        assert_snapped(gate.snap(&m, &req, &scp, &inp_safe, &native).unwrap());
    assert_eq!(packet.decision_view_root, view.root());
    assert_eq!(
        view.root(),
