...
use zero_store::{
    ...
    record_owner_death, recover_attempt, recover_journal, recover_lease_journal,
    verify_committed_lease_binding,
};
...
fn prepare_commit_and_abort_are_idempotent_with_old_or_new_visibility() {
    ...
    assert_eq!(
        read_journal_record(&jp).unwrap().state,
...
fn five_term_binding_tamper_refused_on_commit_and_verify() {
    ...
    assert_eq!(root_before, b.old_root);
    ...
    // Any tampered five-term field must be rejected on commit with a designated mismatch.
    let journal_tampered = fs::read(jp.journal_record()).unwrap();
    let cartridge = read_lease_continuation_cartridge(&jp).unwrap();
    let err = commit_lease_journal(&jp, &cartridge).unwrap_err();
    assert_eq!(err.code, JournalFailureCode::CartridgeMismatch);
    // Published root and journal must remain unchanged after tamper rejection.
...
fn five_term_two_writer_cas_one_success() {
    ...
    let cartridge_a = prepare_lease_journal(&jp, b_a.clone()).expect("prepare A");
    // Concurrent contender from same old root must lose with terminal conflict.
    let err = prepare_lease_journal(&jp, b_b.clone()).unwrap_err();
    ...
    assert_eq!(read_published_root(&jp).unwrap().root_digest, b_a.new_root);
    assert_eq!(
        read_lease_journal_record(&jp).unwrap().state,
        JournalState::Committed
    );
    // Loser's old root is now stale; any further prepare with it must be RootMismatch.
    let err = prepare_lease_journal(&jp, b_b).unwrap_err();
...
fn five_term_second_prepare_and_replayed_lease_cannot_mutate() {
    ...
    assert_eq!(fs::read(jp.journal_record()).unwrap(), journal_bytes_before);
    assert_eq!(fs::read(jp.root_record()).unwrap(), root_bytes_before);
    let committed = commit_lease_journal(&jp, &first).expect("commit first");
    assert_eq!(committed.outcome, RecoveryOutcome::NewRootCommitted);
    let root_after_commit = fs::read(jp.root_record()).unwrap();
    let journal_after_commit = fs::read(jp.journal_record()).unwrap();
    let cartridge_after_commit = fs::read(jp.cartridge()).unwrap();
    assert_eq!(read_published_root(&jp).unwrap().generation, 1);
    assert_eq!(read_published_root(&jp).unwrap().root_digest, b.new_root);
    // Replaying the original committed cartridge must return the same receipt and leave bytes unchanged.
    let replay = commit_lease_journal(&jp, &first).expect("replay same cartridge");
    assert_eq!(replay, committed);
    assert_eq!(fs::read(jp.root_record()).unwrap(), root_after_commit);
    assert_eq!(fs::read(jp.journal_record()).unwrap(), journal_after_commit);
    ...
    assert!(
        ...
        "forged recovery must be ImmutableReceiptConflict or AlreadyTerminal, got {:?}",
        err.code
    );
    assert_eq!(fs::read(jp.root_record()).unwrap(), root_after_commit);
    assert_eq!(fs::read(jp.journal_record()).unwrap(), journal_after_commit);
