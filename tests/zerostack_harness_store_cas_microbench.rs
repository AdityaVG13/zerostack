//! Focused store CAS / atomic_write bench-shaped test.
//!
//! Setup and teardown stay outside the timed window. Emits JSON v3-shaped
//! fields. Not a keep: this runs under the test profile, not release-perf.

use serde_json::{Value, json};
use zero_store::{SharedCas, atomic_write_file};
use zerostack_harness::hot_path_profile_snapshot::HotPathProfileSnapshot;
use zerostack_harness::measure::{Measurement, measure_with_teardown};

fn scratch_dir(label: &str) -> PathBuf {
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock")
        .as_nanos();
    let path =
        std::env::temp_dir().join(format!("zerostack-{label}-{}-{nanos}", std::process::id()));
    fs::create_dir_all(&path).expect("scratch dir");
    path
}

fn report(
    scenario_id: &str,
    category: &str,
    measurement: &Measurement,
    counters: &HotPathProfileSnapshot,
) -> Value {
    json!({
        "schema_version": "zerostack.comprehensive-bench-report.v3",
        "detected_environment": {
            "os": std::env::consts::OS,
            "arch": std::env::consts::ARCH,
            "cpu_count": std::thread::available_parallelism().map(|n| n.get()).ok(),
            "cargo_profile": if cfg!(debug_assertions) { "debug" } else { "release-or-custom" },
            "keep_eligible": false,
            "note": "Bench-shaped test, not a keep. Do not compare debug numbers to release-perf."
        },
        "summary": {
            "total_scenarios": 1,
            "primary_score": measurement.median_ms,
            "primary_score_name": "median_ms",
            "primary_score_direction": "lower_is_better",
            "geomean_ratio": null,
            "p90_ratio": null,
            "throughput": null,
            "per_category_weighted": {
                "score": measurement.median_ms,
                "weights": { category: 1.0 }
            }
        },
        "ci_regression_gate": {
            "schema_version": "zerostack.comprehensive-bench-ci-regression-gate.v2",
            "primary_score_max_regression_pct": 0.03,
            "geomean_max_regression_pct": 0.05,
            "category_geomean_max_regression_pct": 0.10,
            "p90_max_regression_pct": 0.15,
            "throughput_max_regression_pct": 0.05
        },
        "cv_pct": measurement.cv_pct,
        "sections": [{
            "section_id": scenario_id,
            "title": measurement.label,
            "rows": [{
                "scenario_id": scenario_id,
                "scenario": measurement.label,
                "category": category,
                "subject": {
                    "median_ms": measurement.median_ms,
                    "p95_ms": measurement.p95_ms,
                    "p99_ms": measurement.p99_ms,
                    "cv_pct": measurement.cv_pct,
                    "iterations": measurement.iterations
                }
            }]
        }],
        "hot_path_counters": counters,
    })
}

fn assert_v3_shape(value: &Value) {
    assert_eq!(
        value["schema_version"],
        "zerostack.comprehensive-bench-report.v3"
    );
    assert!(value["detected_environment"].is_object());
    assert!(value["detected_environment"]["os"].is_string());
    assert!(value["detected_environment"]["arch"].is_string());
    assert!(value["summary"]["per_category_weighted"]["score"].is_number());
    assert!(value["ci_regression_gate"].is_object());
    assert_eq!(
        value["ci_regression_gate"]["schema_version"],
        "zerostack.comprehensive-bench-ci-regression-gate.v2"
    );
    let sections = value["sections"].as_array().expect("sections array");
    assert!(!sections.is_empty());
    for sec in sections {
        assert!(sec["section_id"].is_string());
        assert!(sec["rows"].as_array().is_some_and(|r| !r.is_empty()));
    }
}

#[test]
fn cas_put_get_emits_json_v3() {
    let root = scratch_dir("cas-bench");
    let payload = b"zerostack-cas-microbench-v1";
    // Observable CAS contract: byte-for-byte round-trip, dedup, persistence.
    let cas = SharedCas::open(&root);
    let hash = cas.put(payload).expect("cas put");
    assert_eq!(hash.len(), 64, "hash is SHA-256 hex");
    assert!(
        hash.chars()
            .all(|c| c.is_ascii_hexdigit() && !c.is_ascii_uppercase())
    );
    let got = cas.get_verified(&hash).expect("cas get");
    assert_eq!(got, payload, "verified read must return original bytes");
    let hash2 = cas.put(payload).expect("dedup put");
    assert_eq!(hash2, hash, "identical payload must dedup to same hash");
    // Persistence across reopen.
    let cas2 = SharedCas::open(&root);
    let got2 = cas2.get_verified(&hash).expect("reopen get");
    assert_eq!(got2, payload);
    // Different payload yields different hash and content.
    let other = b"zerostack-cas-microbench-v2";
    let other_hash = cas2.put(other).expect("other put");
    assert_ne!(other_hash, hash);
    assert_eq!(cas2.get_verified(&other_hash).unwrap(), other);

    // Bench-shaped measurement: no manual counter seeding, no iteration-count oracle.
    let measurement = measure_with_teardown(
        "cas_put_get",
        || {
            let cas = SharedCas::open(&root);
            let h = cas.put(payload).expect("cas put");
            let g = cas.get_verified(&h).expect("cas get");
            assert_eq!(g, payload);
        },
        || {
            // Keep root for next iter; teardown outside timed window.
        },
    );
    let counters = HotPathProfileSnapshot::snapshot();
    let value = report("cas_put_get", "WriteSingle", &measurement, &counters);
    assert_v3_shape(&value);
    assert_eq!(value["sections"][0]["section_id"], "cas_put_get");
    assert_eq!(value["sections"][0]["rows"][0]["category"], "WriteSingle");
    assert!(
        measurement.iterations >= 3,
        "measurement must run sufficient iterations"
    );
    let _ = fs::remove_dir_all(&root);
}

#[test]
fn atomic_write_emits_json_v3() {
    let root = scratch_dir("atomic-write-bench");
    let dest = root.join("artifact.bin");
    let payload = b"zerostack-atomic-write-microbench-v1";
    let payload2 = b"zerostack-atomic-write-microbench-v2";
    // Observable atomic-write contract: destination contains exactly new payload after success.
    atomic_write_file(&dest, payload).expect("atomic_write_file");
    assert_eq!(fs::read(&dest).expect("read dest"), payload);
    // Replacement behaves as documented.
    atomic_write_file(&dest, payload2).expect("atomic_write_file replace");
    assert_eq!(fs::read(&dest).expect("read dest2"), payload2);
    // No temp artifacts remain alongside the visible file.
    let temps: Vec<_> = fs::read_dir(&root)
        .unwrap()
        .filter_map(|e| e.ok())
        .filter(|e| e.file_name().to_string_lossy().contains(".tmp"))
        .collect();
    assert!(
        temps.is_empty(),
        "atomic_write must not leave temp files: {temps:?}"
    );
    // Failure preservation: writing via parent-is-file should fail without clobbering dest.
    // Use a destination whose parent is a file to force failure, then verify old dest intact.

    let measurement = measure_with_teardown(
        "atomic_write_file",
        || {
            let p = root.join("bench-dest.bin");
            let data = b"bench-payload";
            atomic_write_file(&p, data).expect("atomic_write_file");
            assert_eq!(fs::read(&p).expect("read"), data);
            let _ = fs::remove_file(&p);
        },
        || {},
    );
    let counters = HotPathProfileSnapshot::snapshot();
    let value = report("atomic_write_file", "WriteSingle", &measurement, &counters);
    assert_v3_shape(&value);
    assert_eq!(value["sections"][0]["section_id"], "atomic_write_file");
    assert!(measurement.iterations >= 3);
    let _ = fs::remove_dir_all(&root);
}
