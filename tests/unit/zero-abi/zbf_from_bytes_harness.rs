...
fn arb_leaf(
    ...
    (arb_kind(), arb_owner(), arb_payload_bytes()).prop_map(move | (kind, owner, payload) | {
        ZbfObject::new_leaf(kind, owner, assembly, profile, source_root, producer, payload)
            .expect("bounded leaf")
    })
}
...
proptest! {
    ...
    #[test]
    fn roundtrip_bytes_never_panic_on_valid(obj in arb_object()) {
        let profile = DurableProfile::portable_strict();
        let assembly = Sha256Digest::from_bytes([0x11; 32]);
        let bytes = obj.to_bytes(profile).unwrap();
        // Also exercise single-bit mutants of valid bytes still not panicking.
        for off in [0usize, 1, 8, 64, 192] {
            if off < bytes.len() {
                let mut mutated = bytes.clone();
                mutated[off] ^= 0x01;
                let _ = ZbfObject::from_bytes(&mutated, assembly, profile);
            }
        }
    }
}
...
fn e2e_empty_and_bad_magic_are_errors() {
    ...
    for seed in [b"".as_slice(), b"NOTZBF!!", &[0u8; 16], &[0xffu8; 64]] {
        let _ = ZbfObject::from_bytes(seed, assembly, profile);
    }
}
...
fn e2e_wrong_assembly_is_error() {
    ...
    assert_eq!(format!("{err}"), format!("{}", zero_abi::ZbfError::AssemblyMismatch {
        expected: wrong,
        actual: obj.header.assembly_manifest_digest,
    }));
}

#[test]
fn e2e_max_depth_boundary() {
    // Build a chain of single-child containers up to max depth and verify roundtrip + depth error.
    let profile = DurableProfile::portable_strict();
    let assembly = Sha256Digest::from_bytes([0x11; 32]);
    let source_root = Sha256Digest::from_bytes([0x22; 32]);
    let producer = Sha256Digest::from_bytes([0x33; 32]);
    ...
    // Build depth = ZBF_MAX_DEPTH containers nested.
    ...
    // At max depth should still round-trip.
    let bytes = leaf.to_bytes(profile).unwrap();
    let decoded = ZbfObject::from_bytes(&bytes, assembly, profile).unwrap();
    assert_eq!(decoded, leaf);

    // One more nesting should fail at construction (DepthExceeded) not panic.
