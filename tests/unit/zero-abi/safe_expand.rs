...
fn permit_encodes_read_only_authority_only() {
    let issuer = issuer();
    let handle = issuer.issue(&request()).unwrap();
    let ExpandOutcome::Safe(permit) = issuer.revalidate(&handle, &live()) else {
    ...
    for forbidden in [
        ...
        for key in object.keys() {
            assert!(
                ...
                "permit must not encode {forbidden} authority, found key {key}"
            );
        }
    }
    // The permit is exactly the verified read-only binding surface.
    let expected = [
        "handle_id",
        "project_root",
        "request_root",
    ...
    assert_eq!(
        object.len(),
        expected.len(),
        "permit carries exactly the read-only bindings"
    );
}
