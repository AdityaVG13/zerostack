...
#[test]
fn unnormalized_reasons_are_rejected() {
    let mut result = complete_result();
    result.verdict = SafetyVerdict::Unknown {
        reasons: vec!["z_reason".into(), "a_reason".into()],
    };
    result.reasons = vec!["z_reason".into(), "a_reason".into()];
    assert_eq!(
        result.validate(&request()),
        Err(TaskLensError::UnnormalizedReasons)
    );
...
#[test]
fn serde_round_trip_preserves_request_and_result() {
    let request = request();
    let wire = serde_json::to_value(&request).expect("request serializes");
    assert_eq!(
        serde_json::from_value::<TaskLensRequest>(wire).expect("request deserializes"),
        request
    );
    ...
    assert_eq!(
        serde_json::from_value::<TaskLensResult>(wire).expect("result deserializes"),
        result
    );
}
