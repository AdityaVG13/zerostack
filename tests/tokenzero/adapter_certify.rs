...
use zero_abi::{
    CompressionRequest, EngineInvocation, ExpandOptions, ProjectionRequest, TokenEngine,
};
...
/// Opens the engine against the hermetic workspace's shared hub store.
...
#[test]
fn zero_token_conforms_to_shared_contract() {
    let ws = workspace();
    let invocation = invocation_for(&ws);
    let engine = engine(&ws);
...
#[test]
fn binary_projection_does_not_claim_lossy_text_as_source_bytes() {
    let ws = workspace();
    let invocation = invocation_for(&ws);
    let engine = engine(&ws);
    let bytes = vec![0xff; 256];
    ...
    let result = engine
        .project(
            ...
            ProjectionRequest {
                bytes: bytes.clone(),
                visible_byte_limit: 96,
                media_type: "application/octet-stream".into(),
            },
        )
        .unwrap();

    assert_eq!(result.visible_source_bytes, 0);
    assert!(
        !result.visible.contains('�'),
        "lossy replacement characters are not source bytes"
    );
    let handle = result
        .exact
        .expect("binary projection requires an exact handle");
    assert_eq!(
        engine
...
#[test]
fn projection_rejects_a_budget_that_cannot_fit_an_exact_handle() {
    let ws = workspace();
    let invocation = invocation_for(&ws);
    let engine = engine(&ws);
    ...
    let error = engine
        .project(
            ...
            ProjectionRequest {
                bytes: vec![b'x'; 256],
                visible_byte_limit: 8,
                media_type: "text/plain; charset=utf-8".into(),
            },
        )
        .expect_err("a partial ZeroHandle would not be recoverable");

    assert_eq!(error.kind, zero_abi::EngineErrorKind::Budget);
}
